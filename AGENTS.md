...
- **Workspace tools** are bare `<source>__<tool>` — the per-workspace registries, including the platform `nb` source. The workspace is NOT in the name: it comes from the session's membership-validated `workspaceId`, so a caller cannot name another workspace at all. (`ws_<id>-<source>__<tool>` is the LEGACY form — no longer emitted, still routed for resumed conversations and cached external `tools/list`.)
