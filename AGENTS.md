...
- **Less code is the goal.** Each branch is a place a bug can hide. Before adding a safety check or new code path, name the specific scenario it prevents. If you can't, don't add it. A bug fix that adds 50 lines to defend against a scenario that an upstream change could prevent is a smell — fix the cause first.
- **Symmetric defense.** When you add a check on one code path, identify parallel paths that do similar work and apply the same check everywhere — or refactor so there's only one path. Asymmetric gates are how round-2 QA findings appear ("happy path enforces X, new branch doesn't").
- **Refuse on conflict; never auto-merge.** When two data states might collide (a stub at the target id, a file at a rename destination, a member already present), refuse and require operator decision. Auto-merging is how silent data loss happens. Refusal is recoverable; merge is not.
- **Allowlist over denylist for safety gates.** "Fields that are explicitly safe; everything else refuses" defaults future additions to safe. "Fields that block this action" enumerates known-bad and silently allows new fields you didn't anticipate. The script for canonical-stub validation in `scripts/heal-truncated-personal-workspaces.ts:ALLOWED_STUB_KEYS` is the pattern.
- **Prefer fixing the cause over adding a bandaid.** When a script needs a heal, ask: can the upstream script be fixed to not need it? Heal scripts, fallback paths, and special-case branches are technical debt by construction.
- **Before opening a PR, re-read the body against the diff.** Every claim (test count, file list, behavior assertions) must match what landed. Update or strike. The `pr-review` skill checks for body-vs-code drift; catching it before push avoids a QA round.
