...
- **Decompose** a `<source>__<tool>` name only via `splitInnerToolName(s)` from `src/util/tool-name.ts` — the one grammar every dispatch door shares. It lives in `src/util/` because both `src/tools/` and `src/runtime/` need it and `check:cycles` forbids the latter importing the former — a home under `src/tools/` forces a copy. Read the returned `hasSeparator` for "this name has no source segment" — never infer it by comparing the two segments (`x__x` has equal segments *and* a separator), and never re-derive it from a sentinel. Two doors that split a name differently is a misroute, not a wrong label. `test/unit/tools/tool-name-grammar.test.ts` pins the doors against one table.
