...
- **Advertise the MCP tasks capability at `hostCapabilities.experimental["io.modelcontextprotocol/tasks"]`.** The official ext-apps `App` parses the handshake result against the spec schema, which has no `tasks` field and strips one; `experimental` keyed by the extension identifier survives the parse (ext-apps 1.7.5+). Read it from there in any app code. The top-level `tasks` field stays beside it for apps on SDKs that read the raw result; it goes once none do.
...
- Picker uploads (`synapse/request-file`) MUST persist via `POST /v1/resources` (multipart) and answer `{ files: FileEntry[] }`, with `{ files: [] }` when the user cancels. Never a bare array, never bytes: base64-in-`tools/call` arguments hits the 1 MB JSON cap and silently breaks for any binary above ~750 KB.
