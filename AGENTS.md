...
- **Identity tools** (kernel identity sources — `conversations`, …; see `src/tools/identity-sources.ts`) are **bare** `<source>__<tool>`. They're owned by the user and live OUTSIDE any workspace, so they're NOT composed into workspace registries.
