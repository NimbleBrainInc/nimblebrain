...
**Workspace-scoped writes have no org-admin bypass, and the web tier must agree.** `canWriteWorkspaceScoped` (`src/workspace/authz.ts`) allows a write only for a workspace **member** whose membership role is `admin`; `orgRole` is never consulted. The web tier's `useScopedRole` deliberately does the opposite — it escalates an org admin to `org_admin` *before* reading the workspace role — because that is the right answer for **reach** (nav, route guards, read gates), where an org admin legitimately gets to any workspace's settings. So the two must not share a helper: gate a **write** with `useCanWriteActiveWorkspace()` (`web/src/hooks/useScopedRole.ts`), and reserve `roleAtLeast(role, "ws_admin")` for reach. Getting this backwards offers controls the server refuses and surfaces as a 403 on save. It shipped in nine places before being caught, in three different shapes — `roleAtLeast(…, "ws_admin")`, the bypass written longhand as `isOrgAdmin || <membership check>`, and an affordance with no gate at all — so grepping for one shape never establishes that a surface is covered. A workspace-scoped write **should** route through `canWriteWorkspaceScoped`, and a client gate that disagrees with it is a bug — but do not read that as an invariant you can lean on. Known writes that skip it include `handleSetPermissions` (#748), `POST /v1/mcp-auth/initiate` (#755), and `manage_workspaces update`, which patches `workspace.json`'s `bundles` — the field install/uninstall mutate under `isWorkspaceAdmin` — behind an org-admin gate instead. That last one is not a hole (an org admin can delete the workspace outright, and no web caller sends `bundles`), but it means the helper is a convention, not a chokepoint: verify the write path rather than assuming it.
...
**Connection credential re-validation (`ConnectionRevalidator`).** A runtime-owned timer (sibling of `HealthMonitor`, started in `serve`) that detects connectors whose upstream authorization lapsed *without* a transport 401 — managed-MCP providers (Composio) whose downstream vendor account expires while the platform→provider key stays valid. It polls each provider through a generic `ConnectionHealthProbe` (`src/bundles/connection-probe.ts`; Composio's impl is `src/connectors/providers/composio/connection-probe.ts`) and flips `running → reauth_required` after N consecutive `credential_lost` verdicts (anti-flap; any API error/timeout is `indeterminate` = no-op; a flap-storm trips a circuit breaker that keeps all state). Dormant unless a probe is registered (Composio configured). Operator knobs (read once at startup, restart to change): the Composio-probe kill switch `connectors.providers.composio.monitorEnabled` in `nimblebrain.json` (default on when the provider is configured; falls back per field to `COMPOSIO_MONITOR_ENABLED` when left out) and `NB_CONNECTION_REVALIDATE_INTERVAL_SECONDS` (generic sweep cadence, default 300; the legacy `COMPOSIO_MONITOR_INTERVAL_SECONDS` is still honored but deprecated, warns once at startup, slated for removal #727). Does NOT touch transports/restart/`dead` — that stays `HealthMonitor`'s job (liveness-of-process vs. liveness-of-credential, two disjoint loops).
