...
2. `bun run migrate:conversations-to-workspace` — moves each flat `{workDir}/conversations/<convId>.jsonl` under its workspace at `workspaces/<wsId>/conversations/<ownerId>/` (`wsId` from the conversation's `workspaceId`, or the owner's personal workspace when absent; automation runs land in `_runs/<automationId>/`). Dry-run by default; `--write` to apply. Idempotent.
