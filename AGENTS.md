...
**Conversations are room-owned.** Every conversation lives at `workspaces/<wsId>/conversations/<ownerId>/<convId>.jsonl` (automation runs at `…/conversations/_runs/<automationId>/`) and is authorized by ownership (`Conversation.ownerId === access.userId`). The **path is the binding**: `Conversation.workspaceId` is set at create (the room the chat is born in, at the first message) and never mutated — there is no mid-chat room switching — so the directory is authoritative and the field is a denormalised convenience. Construct dirs ONLY via `roomConversationsDir` / `runConversationsDir` from `src/conversation/paths.ts` (the single sanctioned site; the flat top-level `join(workDir, "conversations")` is now the regression `check:conversation-paths` catches). Read one conversation via `runtime.findConversation(convId, { userId })` and list via `runtime.listConversations(options, access)` — both route through the process-wide `ConversationLocator`, which resolves `convId → { wsId, ownerId }` across rooms and serves the room-scoped (`options.workspaceId`) and "All rooms" (owner-only) views; write via `runtime.roomConversationStore(wsId, ownerId)` / `runConversationStore(wsId, automationId)`. Deleting a workspace **archives** its subtree to `archived/<wsId>/` (archive-then-cascade), never a hard `rm`. **Personal workspace ids** go through `personalWorkspaceIdFor(userId)` from `src/workspace/workspace-store.ts` — no hand-built `"ws_user_" + userId` or `` `ws_user_${userId}` `` outside that helper (`check:personal-workspace-id` enforces).

**Files are room-owned.** A file lives at `workspaces/<wsId>/files/<ownerId>/<fileId>_<name>` (per-owner registry + bytes + sidecars in that partition), so the directory is the boundary: cross-room reads fail by construction and `workspace delete` archives files with the rest of the room. Build a store ONLY via `runtime.getRoomFileStore(wsId, ownerId)` (which constructs through `roomFilesDir` from `src/files/paths.ts`, the single sanctioned site; `check:file-paths` rejects the identity-scoped `getIdentityContext(...).getDataPath("files")`). A `files://<id>` URI stays **bare** — the room is NOT in the URI; it comes from the ambient request. Because `files__*` is an identity-door tool but `scope.workspaceId` is the personal/session workspace, the focused room rides a dedicated `RequestContext.fileWorkspaceId` (set on both doors: chat = `request.workspaceId ?? sessionWsId`; REST/`/mcp` = the validated `X-Workspace-Id`). No room in scope ⇒ file storage denies (e.g. an external `/mcp` call with no header). The browser serve endpoint carries the room as a query param: `GET /v1/files/:id?ws=<wsId>` (the web `fileUrl()` helper appends it). `_runs/<automationId>/` is reserved for automation outputs (PR 3). `FileEntry.ownerId`/`workspaceId` are denormalised — the path is authoritative. Operators run `bun run migrate:files-to-room` before deploying.

**Automations are identity-owned, not workspace-scoped** (Phase C; their room relocation is a later PR). Automations live owner-partitioned at `{workDir}/users/{userId}/automations/` — resolve the dir via `runtime.getIdentityContext(userId).getDataPath("automations")` (`check:automation-paths` rejects the workspace-scoped path). The scheduler is multi-owner (scans `users/*/automations/`, keys by `${ownerId}/${id}`) and a scheduled run **fires as its owner** — an identity-bound session **walled to** the automation's provenance `workspaceId` (its tools + the owner's identity tools), with NO cross-workspace reach. Membership in the provenance workspace is validated when the automation is created (the wsId is stamped from the creator's trusted context), not re-checked per run — so revoking an owner's membership does not stop their existing automations from firing against that workspace (offboarding is a separate concern). (Automation room-relocation, with its acts-as / run-visibility decisions, is a later PR — until then they stay identity-owned.)

### Workspace tool namespacing — the wall

A chat or task session reaches **exactly one workspace** plus the caller's identity tools — never a cross-workspace union. Reaching another workspace does not exist; it is **denied, not gated**. **A tool name's shape is its scope (two doors):**
...
**Skills are walled the same way.** Layer-3 skill selection (`selectRequestLayer3`) loads org-tier (`workDir/skills/`, org-wide), workspace-tier (the focused `wsId` only), and user-tier (`users/<userId>/skills/`) skills — plus **bundle skills** (a connector/app's own `skill://<name>/usage` guidance, synthesized and tool-affinity-matched) from the **focused workspace only**. A bundle installed in another workspace never injects its skill here. No skill crosses a workspace boundary. The **app-aware briefing** is walled the same way: `focusedApp` / `<app-guide>` / `<app-state>` resolve `appContext.serverName` only in the session's bound workspace (`focusedWsId ?? sessionWsId`), never by scanning the identity's other workspaces — so the prompt never describes an app whose tools the wall would refuse to call.
...
2. `bun run migrate:conversations-to-room` — moves each flat `{workDir}/conversations/<convId>.jsonl` under its room at `workspaces/<wsId>/conversations/<ownerId>/` (`wsId` from the conversation's `workspaceId`, or the owner's personal workspace when absent; automation runs land in `_runs/<automationId>/`). Dry-run by default; `--write` to apply. Idempotent.
