...
**`/mcp` is identity-only — and the synapse iframe bridge is BROKEN.** A `/mcp` session carries no workspace (the `X-Workspace-Id` header is read but NOT honored), so `tools/list` returns only identity tools, any `ws_<id>-` call is refused (`WorkspaceToolUnavailable`), and the `tasks` capability is not advertised. **Known limitation, accepted intentionally:** the first-party iframe bridge (`web/src/bridge/bridge.ts`) dispatches synapse-app *workspace* tools over `/mcp` as `ws_<activeWsId>-…` (and sends `X-Workspace-Id`), so **synapse-app tool calls over `/mcp` fail today.** The fix is to wall `/mcp` to the validated `X-Workspace-Id` — a workspace-bound "agent projection" — tracked as follow-up OUTSIDE this change. Do not "restore" the old cross-workspace `/mcp` union to fix the bridge; that reopens the hole this wall closed.
...
> ⚠️ **`/mcp` is identity-only today, and rows 1–2 are degraded.** A `/mcp` session carries no workspace, so it serves only identity tools (conversations/files/automations) and refuses any `ws_<id>-` workspace-tool call. External clients lose all workspace tools; the **iframe bridge's synapse-app tool calls fail** (it dispatches `ws_<activeWsId>-…` over `/mcp`). This is an accepted interim — see "Workspace tool namespacing — the wall" above. The fix (wall `/mcp` to the validated `X-Workspace-Id`) is follow-up.
