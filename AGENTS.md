...
- **An iframe reaches its own server and no other.** `tools/call`, `resources/read`, `resources/list` and `resources/templates/list` go to the server that rendered the view, whatever the app names: a qualified tool name for another server is refused, and a read names the app's server on the wire so `/mcp` never resolves the URI across the workspace. Never add a parameter, `_meta` key or name form that picks a different server.
...
- **Advertise the MCP tasks capability at `hostCapabilities.experimental["io.modelcontextprotocol/tasks"]`.** The official ext-apps `App` parses the handshake result against the spec schema, which has no `tasks` field and strips one; `experimental` keyed by the extension identifier survives the parse (ext-apps 1.7.5+). Read it from there in any app code.
