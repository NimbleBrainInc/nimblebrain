...
- Focus is derived from the **URL slug**, never from `WorkspaceContext`'s `activeWorkspace`. That state starts on bootstrap's default (the personal workspace) and reconciles to the route a render later; keying focus off it makes that intermediate value look like a workspace switch, which clears the conversation the per-tab restore just reopened. Membership-gate the slug so an unknown one holds at `null`.
