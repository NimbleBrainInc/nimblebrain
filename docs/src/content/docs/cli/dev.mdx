...
To iterate on a Synapse app's UI with hot module replacement, point `bun run dev` at the app directory with `--app`. The CLI reads the app's local `manifest.json` for its name, registers that name in dev mode so its `ui://` resources are served from Vite, and starts the dev server (default port `5173`). Run the app's MCP server yourself and point the workspace at its URL — see [Local Development](/apps/local-dev/):
...
- [Running the server](/cli/serve/) -- production server (no file watching)
