...
The runtime has no terminal client. You interact with a running instance through the web app it serves and the `/mcp` endpoint (for external MCP clients) -- not from the shell. Connectors, skills, credentials, and automations are managed from the web UI and the agent's tool actions, not from subcommands.
