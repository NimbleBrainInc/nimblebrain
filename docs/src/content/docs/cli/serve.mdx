...
`bun run start` launches the NimbleBrain runtime and starts an HTTP API server. This is the production entry point -- it boots the agent engine, loads all configured connectors, and listens for HTTP requests.
...
2. The runtime initializes -- loading connectors, skills, and model configuration
...
- [Dev mode](/cli/dev/) -- development mode with file watching and HMR
