...
NimbleBrain exposes a `/mcp` endpoint that turns the platform into a Streamable HTTP MCP server. External MCP clients — Claude Code, Open WebUI, or another NimbleBrain instance — can connect and access your identity tools (conversations, files, automations) through a single endpoint. Workspace-scoped tool access over `/mcp` is being reworked into a workspace-bound projection; until it lands, `/mcp` serves identity tools only.
