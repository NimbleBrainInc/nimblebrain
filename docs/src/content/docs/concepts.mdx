...
NimbleBrain installs no connectors by default. Platform capabilities (home, conversations, files, settings, usage, automations) are built in as inline tool sources. Install connectors from the catalog to give the agent domain-specific tools.
...
See [Connector Configuration](/config/bundles/) for the full field reference.
