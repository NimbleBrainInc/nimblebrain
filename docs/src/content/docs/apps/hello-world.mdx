...
- Hot-reload development with `bun run dev --app`
...
function HelloApp() {
  ...
  // Refresh when our server announces a write
...
| `useDataSync(callback)` | Fires when your server announces a write with `notifications/resources/list_changed` |
...
Run your server over HTTP, point a dev workspace at its URL, and start the
platform with `--app` so the UI is served from Vite:

```bash
# Terminal 1
uv run python -m mcp_hello.server --http --port 8765
# Terminal 2, in the platform repo
bun run dev --app ./ui
```

See [Local Development](/apps/local-dev/) for the workspace entry and the
`allowInsecureRemotes` switch loopback URLs need. You get everything the preview
gives you plus data sync, agent interactions, and multi-app navigation.
