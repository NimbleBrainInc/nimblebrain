...
Returns a JSON envelope in the shape of the MCP `ReadResourceResult`, so a client reads the protocol directly — including `_meta` (e.g. ext-apps `_meta.ui.csp`) — without a translation layer. `POST /v1/resources/read` returns the same shape.
...
- **Theme tokens** — Apply the host's CSS custom properties from the `ui/initialize` response's `hostContext`, and again on `ui/notifications/host-context-changed`. See [MCP App Bridge](/apps/bridge) for the protocol.
...
NimbleBrain's own surfaces — the home dashboard, conversation list, files, and automations — use the exact same `ui://` pattern as third-party apps. There is no separate "core" code path. Each built-in is an **in-process MCP server**: it runs inside the platform process (over an in-memory transport) and registers `ui://` resources just like a remote connector does.
...
```
GET /v1/apps/home/resources/primary   → home app's ui:// resource
GET /v1/apps/files/resources/primary  → files app's ui:// resource
```
