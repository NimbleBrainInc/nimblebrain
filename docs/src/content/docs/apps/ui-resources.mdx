...
```
GET /v1/workspaces/ws_acme/apps/synapse-crm/resources/crm/main
```
...
```
GET /v1/workspaces/ws_acme/apps/synapse-crm/resources/primary
...
```
GET /v1/workspaces/ws_acme/apps/synapse-crm/resources/crm/main
GET /v1/workspaces/ws_acme/apps/synapse-crm/resources/crm/settings
GET /v1/workspaces/ws_acme/apps/synapse-crm/resources/crm/contact-detail
```
...
NimbleBrain's own surfaces — the conversation list, files, and automations — use the exact same `ui://` pattern as third-party apps. There is no separate "core" code path. Each built-in is an **in-process MCP server**: it runs inside the platform process (over an in-memory transport) and registers `ui://` resources just like a remote connector does.
...
```
GET /v1/workspaces/ws_acme/apps/files/resources/primary  → files app's ui:// resource
```
