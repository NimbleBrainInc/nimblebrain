...
2. **Add your bundle** by path in `nimblebrain.json`:
   ...
   Then restart the dev server to load it (`nb dev` picks up source changes automatically).
...
<Aside type="tip">
During development, use `{ "path": "../my-app" }` in your config so NimbleBrain loads the bundle from disk. Changes to your MCP server take effect after the dev server restarts (`nb dev` restarts the API on source changes).
</Aside>
