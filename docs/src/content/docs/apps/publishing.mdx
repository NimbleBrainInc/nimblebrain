...
After publishing, anyone can install your app from the Apps catalog in their workspace settings, or by adding it to their `workspace.json` bundles and restarting the server.
...
2. **Add your bundle** by path in `nimblebrain.json`:
   ...
   Then restart the dev server to load it (`bun run dev` picks up source changes automatically).

3. **Verify tools** are registered by asking the agent which apps are installed, or by checking the workspace **Settings → About** tab in the web UI.
...
<Aside type="tip">
During development, use `{ "path": "../my-app" }` in your config so NimbleBrain loads the bundle from disk. Changes to your MCP server take effect after the dev server restarts (`bun run dev` restarts the API on source changes).
</Aside>
