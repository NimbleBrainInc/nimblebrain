...
`X-User-Id`). These are stripped, because the only identity on a forwarded
delivery must be the one the platform verified. If your vendor signs on one of
those headers, declare a rename:
