...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `host_version` | `string` | Yes | Host extension contract version: `"1.0"`, `"1.1"`, `"1.2"`, or `"1.3"`. `1.1` adds the `host_capabilities` block, `1.2` adds `hooks`, and `1.3` adds `notifications` — set the version that matches the newest block you declare. |
