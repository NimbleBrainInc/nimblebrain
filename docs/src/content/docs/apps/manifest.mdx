...
The host checks this against your advertised `tools/list` **at install time** and
refuses the install if the tool is missing or does not accept both arguments. A
stream whose registration tool is wrong would otherwise fail silently, months
