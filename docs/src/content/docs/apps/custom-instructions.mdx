...
- [The workspace overlay](/guide/settings/) — managed by the host, not by your bundle. Set via the `instructions__write_instructions` system tool or the workspace settings UI. Guidance that should reach every workspace in the organization is an org-tier skill instead.
