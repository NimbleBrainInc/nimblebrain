...
INSTRUCTIONS_FILE = Path(os.environ.get("APP_STATE_DIR", "./workspace")) / "custom-instructions.md"
...
- **Composability.** The pattern stacks with `<app-instructions>` (your bundle author's static guidance) and the host's workspace overlay. The system prompt layers cleanly: workspace policy → per-app author guidance → per-app user instructions.
...
- [The workspace overlay](/guide/settings/) — managed by the host, not by your bundle. Written by workspace admins in the workspace settings UI; the agent drafts suggested text but does not write the overlay itself. Guidance that should reach every workspace in the organization is an org-tier skill instead.
