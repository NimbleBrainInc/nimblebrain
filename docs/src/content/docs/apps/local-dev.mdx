...
| **bun run dev --app** | Full NimbleBrain platform with HMR | Testing data sync, agent interactions, multi-app |
| **bun run dev + reload** | Full platform, manual restart | Quick tool changes, no UI build step |
...
| Data sync (agent tool calls) | No — use `bun run dev` |
| Multi-app navigation | No — use `bun run dev` |
...
### With UI hot-reload (`bun run dev --app`)
...
Run your MCP server yourself over HTTP, and point the workspace at it. The
platform connects to remote MCP servers and never spawns one, so a dev loop is
the same shape as production: a URL your workspace holds.

```bash
# Terminal 1 — your server, over streamable HTTP
uv run python -m mcp_myapp.server --http --port 8765
```

Add it to the dev workspace's `workspace.json`, and allow the loopback host
(the SSRF guard refuses it otherwise):

```json title="<workDir>/workspaces/<wsId>/workspace.json"
{
  "connectors": [
    { "url": "http://localhost:8765/mcp", "serverName": "myapp" }
  ]
}
```

```json title="<workDir>/nimblebrain.json"
{ "allowInsecureRemotes": true }
```

<Aside type="caution">
  `allowInsecureRemotes` disables the loopback / RFC1918 / cloud-metadata
  refusal for every connector in the instance. It is a development switch —
  never set it on a deployed platform.
</Aside>
...
The platform connects to the `url` in the workspace config. UI changes require:

<Steps>

1. Rebuild the UI: `cd ui && npm run build`
2. Restart your MCP server so it serves the rebuilt `ui://` resource
...
4. **Write your React app** with Synapse hooks:
   ...
   function MyApp() {
     ...
     useDataSync(() => { /* refetch when the server announces a write */ });
...
Then restart the dev server so the change takes effect (`bun run dev` restarts the API on source changes).
...
In the Synapse Preview, `ui/message`, `ai.nimblebrain/action`, and `ui/update-model-context` messages are logged to the browser console automatically.
