...
2. Restart the dev server so it reloads the bundle (`nb dev` picks up source changes automatically)
...
Then restart the dev server so the change takes effect (`nb dev` restarts the API on source changes).
