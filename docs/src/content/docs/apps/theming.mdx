...
| An `--nb-*` color doesn't flip on toggle | Drive light/dark-sensitive surfaces from the spec `--color-*` tokens |
| Tokens are stale after restart | Delete `deps/<your-package>` during local development (see [Local Development](/apps/local-dev)) |
