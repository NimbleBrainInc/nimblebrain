...
An app that only ever runs inside the platform should use the bare form above, so that a missing token fails visibly rather than resolving to a plausible wrong colour.
