...
2. **Answers your app's `ui/initialize` request** with the spec tokens under `hostContext.styles.variables`.
3. **Sends `ui/notifications/host-context-changed`** when the user toggles dark mode, with updated token values under `styles.variables`, once your app has sent `ui/notifications/initialized`.

<Aside>
The values are not fixed. A deployment can set its own brand, and the tokens your app reads are then that brand's colors, typefaces and corner radius — a different accent, a warm background, square corners. Read the accent from `--color-text-accent` and never assume NimbleBrain blue; a hardcoded hex that matches the default palette will clash with every branded deployment.
</Aside>
...
<Aside type="caution">
**A fallback is a second copy of a value the host owns**, and a copy goes stale when the palette moves.

The failure is quiet and one-sided. A token name that does not exist falls through to the fallback and renders a plausible colour instead of breaking visibly, so the mistake ships. And because fallbacks get written from the light palette, what ships is a light value that only looks wrong in dark mode.
</Aside>
...
The injected `<style>` block gives you the right tokens at load time. When the user toggles dark mode mid-session, the host sends the updated values in `ui/notifications/host-context-changed`, but only to an app that has completed the [initialization handshake](/apps/bridge/#initialization-handshake): the host holds every notification until the app sends `ui/notifications/initialized`. Synapse performs the handshake and applies the tokens for you. Without it, do both yourself:
...
The rule, rather than a list you would have to keep checking: **only keys in the ext-apps spec's own variable enum cross the boundary.** Two families of what the host injects are not in it, so neither refreshes:

- the `--nb-*` extension tokens, which have no spec equivalent by definition, and
- NimbleBrain's additions to spec-shaped families — `--color-text-accent`, and the `3xs`, `2xs` and `base` steps of the type scale (the spec enum carries only `xs`, `sm`, `md` and `lg`).
...
<Aside type="tip">
These tables list the tokens, not their values. The values are whatever the host injects for the active theme and change with it — read them at runtime from the iframe's `:root` rather than hard-coding them anywhere, a `var()` fallback included.
</Aside>
...
<Aside type="note">
The host injects font *names*, not font *files*. A `srcdoc` iframe is its own document and inherits no `@font-face` from the shell, so unless your app loads the face itself the value resolves to the fallback stack. Design against the fallback and treat the brand face as an enhancement — or load it in your own app.
</Aside>
