...
<Aside type="note">
`synapse/request-file` is a NimbleBrain platform extension — it is not part of the ext-apps specification. With the `@nimblebrain/synapse` SDK, call `pickFile()` for a single entry or `pickFiles()` for the list; both read this one shape.
</Aside>
