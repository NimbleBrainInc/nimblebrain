...
3. **Resource scoping** — `resources/read`, `resources/list` and `resources/templates/list` answer from the app's own MCP server. Every iframe shares one `/mcp` session, so the bridge names that server on the request; the iframe cannot name another. A URI another server serves reads as not found, the same as one that does not exist.
