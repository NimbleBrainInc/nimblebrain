...
1. **Workspace scoping** — The host reaches the platform over the workspace's own MCP endpoint, `/mcp/<workspaceId>`, for the workspace on screen. A session belongs to that one workspace: switching workspace closes it and opens one on the new workspace's path, so a request for one workspace never travels on another's session.
