...
// 2. Host responds
{
  ...
  "result": {
    ...
    "hostCapabilities": {
      ...
      "experimental": {
        "ai.nimblebrain/tasks": { "cancel": {}, "requests": { "tools": { "call": {} } } }
      }
    },
    "hostContext": {
      "theme": "dark",
...
The MCP tasks capability appears twice. `hostCapabilities.tasks` is read off the raw response. `hostCapabilities.experimental["ai.nimblebrain/tasks"]` is for a client that validates the response against the ext-apps schema, such as the official `App`: that schema drops the unrecognized `tasks` field and, from ext-apps 1.7.5, keeps the contents of `experimental`. An app on an earlier ext-apps version sees neither.
