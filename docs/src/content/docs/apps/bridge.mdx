...
The MCP tasks capability appears three times, and none of the copies is redundant yet.

`hostCapabilities.tasks` is read off the raw response. The two `experimental` keys are for a client that validates the response against the ext-apps schema, such as the official `App`: that schema drops the unrecognized `tasks` field and, from ext-apps 1.7.5, keeps the contents of `experimental`. An app on an earlier ext-apps version sees none of them.

Read **`io.modelcontextprotocol/tasks`** — the identifier MCP Tasks is registered under as an official MCP extension. `ai.nimblebrain/tasks` is the name this host advertised first and keeps until consumers have moved; prefer the extension identifier and fall back to it.
