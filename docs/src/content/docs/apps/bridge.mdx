...
Ask to be shown `inline`, `fullscreen` or `pip`. The host decides placement from its own layout and does not hand that decision to an app, so the result reports the mode **actually in effect** — which is usually not the one requested.
...
3. **Listing scoping** — `resources/list` and `resources/templates/list` answer from the app's own MCP server. Every iframe shares one `/mcp` session, so the bridge names that server on the request; the iframe cannot name another.
