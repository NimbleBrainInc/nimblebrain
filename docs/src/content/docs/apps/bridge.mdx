...
The host answers the request with an empty result. Sent without an `id` it is a notification, which takes no answer — an app on an older SDK sends it that way.
...
The host answers `{}` once the tab is open, and `{ "isError": true }` when the browser refused to open it — a popup blocker, usually — so an app can navigate itself instead of waiting. Sent without an `id` it is a notification, which takes no answer.
