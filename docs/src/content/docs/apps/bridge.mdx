...
<Aside type="note">
Tool calls are scoped to your app's MCP server. The bridge automatically routes `tools/call` with your app's server name. An iframe cannot call tools on other servers — naming one in `_meta["ai.nimblebrain/server"]` changes nothing for an app that is not built into the shell.
</Aside>
