...
| `notifications/resources/list_changed` | Notification | The app's MCP server announced that the resources it serves changed. See [below](#notificationsresourceslist_changed). |
...
Sent when the app's own MCP server sends `notifications/resources/list_changed`: the server announced that what it serves changed. The host forwards it to that server's views in the workspace where it happened, as the MCP Apps spec defines under `serverResources.listChanged`. Re-read what you show.
...
Several announcements inside the host's 100 ms debounce window arrive as one: the notification carries no payload, so repeats add nothing.
