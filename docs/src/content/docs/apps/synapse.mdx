...
The **host's reply** shape moved too, and an SDK reads only one of them: see [`synapse/request-file`](/apps/bridge#synapserequest-file) for which release reads which, and why an app on 0.18.0 or earlier has to move its pin in the same deploy as the host.
</Aside>
