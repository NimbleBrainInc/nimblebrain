...
When another view of the same app needs to hear about a write (a sidebar list after an inline view saves), have your server send `notifications/resources/list_changed`. The host relays it verbatim to that server's views — see [the bridge reference](/apps/bridge#notificationsresourceslist_changed).
</Aside>
