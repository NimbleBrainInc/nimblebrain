...
Synapse is the recommended way to build NimbleBrain app UIs. It wraps the standard [MCP Apps protocol](/apps/bridge) and adds typed tool calls, LLM-visible state, data sync, and keyboard forwarding.
...
See the SDK guide, [Keep the UI in sync with the agent](https://synapse.nimblebrain.ai/docs/guides/keep-ui-in-sync/), for the server side in Python and TypeScript, and [the bridge reference](/apps/bridge#notificationsresourceslist_changed) for what the host relays.
...
<Aside type="note">
`action()` and `pickFile()` / `pickFiles()` work only where the host declares the matching `ai.nimblebrain/` extension: `action()` is a silent no-op otherwise and the picker throws `HostCapabilityError`, since it depends on host-side persistence. `chat()` sends nothing without the `message` capability, and `openLink()` falls back to `window.open()` without `openLinks`. Read `app.hostCapabilities` to decide whether to render a control at all.
</Aside>
...
The **method name and the host's reply shape** both moved again in 0.20.0: the extension is [`ai.nimblebrain/request-file`](/apps/bridge#ainimblebrainrequest-file), answered `{ files }`. A host serves one set of names, so an app's pin and the host's release move in the same deploy.
</Aside>
...
| Export | Contents |
|--------|----------|
| `@nimblebrain/synapse` | `connect`, `createSynapse`, all types |
| `@nimblebrain/synapse/react` | `AppProvider`, `useApp`, `useToolResult`, `useToolInput`, `useResize`, `useConnectTheme`, `SynapseProvider`, `useSynapse`, `useCallTool`, `useDataSync`, `useTheme`, `useAction`, `useAgentAction`, `useChat`, `useVisibleState`, `useFileUpload` |
...
| `useDataSync(cb)` | Connect API | — | `notifications/resources/list_changed` from the app's server |
