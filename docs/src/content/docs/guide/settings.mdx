...
- **Off** — requests no reasoning. Not currently enforceable on adaptive-only Anthropic models, which includes the default: the provider drops the flag, so those models apply their own default ([#738](https://github.com/NimbleBrainInc/nimblebrain/issues/738)).
