...
- **Default** — the platform chooses per model: a capped thinking budget where the model accepts one, otherwise adaptive so the model sets its own depth. Models without reasoning get none.
- **Off** — requests no reasoning. Not currently enforceable on adaptive-only Anthropic models, which includes the default: the AI SDK adapter accepts the flag and then never sends it, so those models apply their own default ([#738](https://github.com/NimbleBrainInc/nimblebrain/issues/738)).
- **Adaptive** — the model decides per call.
- **Enabled** — request reasoning on every turn. Enabling this reveals a **Thinking Budget Tokens** field (minimum 1024), which counts toward Max Output Tokens. On adaptive-only models with no budget or output ceiling set — the out-of-box default — this behaves the same as **Adaptive**.
