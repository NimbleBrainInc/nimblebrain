...
<Aside type="caution" title="Identity tools only (interim)">
A `/mcp` session is identity-bound and carries **no workspace**, so it serves only your identity tools (conversations, files, automations). A `tools/call` for a workspace tool (`ws_<id>-<source>__<tool>`) is refused with `-32602 workspace_access_denied`. The `X-Workspace-Id` header is ignored (logged once at debug under `NB_DEBUG=mcp`). Workspace-scoped tool access is being reworked into a workspace-bound projection; until it lands, `/mcp` exposes identity tools only.
</Aside>
...
In the current interim, the endpoint exposes **identity tools only**:

- **Identity tools** — `tools/list` returns your identity-owned tools (conversations, files, automations). Workspace/bundle tools are **not** exposed over `/mcp` for now.
- **Tool execution** — `tools/call` runs an identity tool. A workspace-namespaced name (`ws_<id>-…`) is refused with `-32602 workspace_access_denied`.
