...
A `/mcp` session is identity-bound, but **each request names its workspace** via the `X-Workspace-Id` header — validated against your membership. `tools/list` returns that workspace's tools (`<source>__<tool>`) plus your identity tools (also `<source>__<tool>`; your personal connectors carry a `my_` marker); a `tools/call` to any **other** workspace is refused with `-32602 workspace_access_denied`. A request with no (or a non-member) `X-Workspace-Id` is identity-only.
</Aside>
...
- **Tool execution** — `tools/call` runs a tool in that workspace or an identity tool. A name in a **different** workspace is refused with `-32602 workspace_access_denied`; with no `X-Workspace-Id`, only identity tools are exposed.
