...
<Aside type="note" title="Per-request workspace">
A `/mcp` session is identity-bound, but **each request names its workspace** via the `X-Workspace-Id` header — validated against your membership. `tools/list` returns that workspace's tools (`<source>__<tool>`) plus your identity tools (also `<source>__<tool>`; your personal connectors carry a `my_` marker); no tool name can address another workspace — the older `ws_<id>-<source>__<tool>` form is retired, and presenting one is refused with `-32602 invalid_tool_name`. A request with no (or a non-member) `X-Workspace-Id` is identity-only, and a workspace tool is refused with `-32602 workspace_access_denied`.
</Aside>
...
For the request's validated `X-Workspace-Id` workspace, the endpoint exposes:

- **Workspace tools** — `<source>__<tool>`, from the connectors installed and running in that workspace. The workspace is not in the name: it comes from the `X-Workspace-Id` you sent, so the same call means a different workspace's tool under a different header.
- **Identity tools** — your conversations, files, and automations (bare `<source>__<tool>`).
- **Tool execution** — `tools/call` runs a tool in that workspace or an identity tool. A name cannot address a **different** workspace; the retired `ws_<id>-` form is refused with `-32602 invalid_tool_name`. With no `X-Workspace-Id`, only identity tools are exposed and a workspace tool is refused with `-32602 workspace_access_denied`. A bare name resolves against that workspace's sources; your personal connectors are reachable only under their `my_`-marked names. A bare name naming neither is refused with `-32602 unknown_tool_source`.
