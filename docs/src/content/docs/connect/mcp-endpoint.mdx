...
A `/mcp` session is identity-bound, but **each request names its workspace** via the `X-Workspace-Id` header — validated against your membership. `tools/list` returns that workspace's tools (`ws_<id>-<source>__<tool>`) plus your identity tools (bare `<source>__<tool>`); a `tools/call` to any **other** workspace is refused with `-32602 workspace_access_denied`. A request with no (or a non-member) `X-Workspace-Id` is identity-only.
</Aside>
...
- **Workspace tools** — namespaced `ws_<id>-<source>__<tool>`, from the bundles installed and running in that workspace.
