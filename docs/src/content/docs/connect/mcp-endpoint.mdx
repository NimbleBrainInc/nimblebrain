...
- **Tool execution** — `tools/call` runs a tool in that workspace or an identity tool. A name cannot address a **different** workspace; the retired `ws_<id>-` form is refused with `-32602 invalid_tool_name`. With no `X-Workspace-Id`, only identity tools are exposed and a workspace tool is refused with `-32602 workspace_access_denied`. A bare name that matches one of your personal connectors is refused with `-32602 personal_connector_requires_marker`, naming the `my_`-marked form to call instead.
