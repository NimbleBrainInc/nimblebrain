...
A NimbleBrain `/mcp` session is bound to your **identity**, but each request names its workspace with the `X-Workspace-Id` header — validated against your membership. `tools/list` returns that workspace's tools (named `<source>__<tool>`, for example `crm__search`) plus your identity tools (`conversations__*`, `files__*`, `automations__*`). Your own personal connectors carry a `my_` marker (`my_gmail__send`) so they stay distinguishable from a workspace connector of the same name — they are different accounts. No tool name can address another workspace: the older `ws_<id>-<source>__<tool>` form is retired, and presenting one is refused with `-32602 invalid_tool_name`. A request with no (or a non-member) `X-Workspace-Id` reaches identity tools only, and a workspace tool is refused with `-32602 workspace_access_denied`.
