...
NimbleBrain extends the spec with `synapse/` namespace methods for data-change notifications, semantic actions, file transfer, and keyboard forwarding. These degrade gracefully to no-ops in non-NimbleBrain hosts.
...
<CardGrid>
  ...
  <LinkCard title="Notifications" description="Expose an outbox so facts your server learns on its own reach a human" href="/apps/notifications" />
