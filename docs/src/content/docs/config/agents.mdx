...
| `model` | `string` | No | Workspace default | Model override. Accepts a slot name (`"default"` or `"fast"`), the same slot written explicitly as `"alias:fast"`, or a `provider:model-id` string. Name a model directly when an agent needs a more capable one than the workspace default. A bare name that is neither a slot nor a known model is routed to Anthropic and fails at the provider — see [models](/config/nimblebrain-json#models). |
