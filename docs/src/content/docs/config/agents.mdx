...
| `model` | `string` | No | Workspace default | Model override. Accepts a slot name (`"default"`, `"fast"`, `"reasoning"`), the same slot written explicitly as `"alias:fast"`, or a `provider:model-id` string. A bare name that is neither a slot nor a known model is passed through to the provider as-is and logs a warning — see [models](/config/nimblebrain-json#models). |
