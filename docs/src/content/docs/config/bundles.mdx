...
<Aside type="tip">
  For sensitive values (API keys, tokens, connection strings), don't put them in `env` here. Set them from [workspace settings in the web UI](/config/credentials/) or a host env var that matches the bundle's `mcp_config.env` declaration — see [Credentials](/config/credentials/) for the full picture.
</Aside>
