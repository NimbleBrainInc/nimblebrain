...
description: Per-workspace configuration — bundles, skill directories, and optional overrides.
---

import { Aside } from '@astrojs/starlight/components';

Each workspace has its own config file at `<workDir>/workspaces/<wsId>/workspace.json`. This is where bundles, skill directories, and optional model or identity overrides live — **not** in [`nimblebrain.json`](/config/nimblebrain-json).
