...
description: Per-workspace configuration — connectors, skill directories, and optional overrides.
---

import { Aside } from '@astrojs/starlight/components';

Each workspace has its own config file at `<workDir>/workspaces/<wsId>/workspace.json`. This is where connectors, skill directories, and optional model or identity overrides live — **not** in [`nimblebrain.json`](/config/nimblebrain-json).
...
| `connectors` | `array` | Yes | Remote MCP connectors installed in this workspace. See [Connector Configuration](/config/connectors). |
...
| `notifications` | `object` | No | Notification source ceilings and delivery routes. `notifications.sources` maps a connector's server name to `{ "maxLevel": "info" \| "attention" \| "urgent" }` — the highest level that source's items may reach a route at, defaulting to `info` for a source nobody has raised. `notifications.routes` is an array of `{ id, createdBy, match, deliver }`. Written only by a workspace admin through **Settings → Notifications**; see [Notifications](/using/notifications). |
...
`notifications` holds the operator half of the notifications contract: which of the workspace's connectors may report how loudly, and where a matching item goes. Both are edited under **Settings → Notifications** — see [Notifications](/using/notifications) for what each field means to an operator.
        ...
        "deliver": [
          {
            ...
            "input": { "channel": "#outbound", "text": "{{title}}\n{{body}}" }
          }
        ]
      }
    ]
...
Two workspaces that install the same server hold independent connections and independent credentials. Each gets its own host-side data directory at `<workDir>/workspaces/<wsId>/data/<serverName>/`, so nothing crosses the workspace boundary. Sidebar placements, briefing facets, and the app list are filtered per workspace.

<Aside type="tip">
  You rarely edit `workspace.json` by hand under auth. Use `nb__manage_workspaces` (admin-only) to create workspaces and update membership; install connectors through the connectors catalog (`manage_connectors`) or the CLI; use the workspace-scoped skill tools to manage `skillDirs`.
</Aside>
...
- [Connector Configuration](/config/connectors) — connector entry fields
