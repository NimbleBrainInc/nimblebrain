...
| `connectorDiscovery` | `true` | Search registries for connectors | `nb__search` (`scope: "registry"`) |
...
<Aside type="tip">
Use feature flags to lock down a production deployment. For example, disable `skillManagement` to prevent the agent from mutating skills, or disable `connectorDiscovery` to keep the agent out of the registry, while leaving the rest of the platform intact.
</Aside>
