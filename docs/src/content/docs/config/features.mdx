...
<Aside type="note">
  `bundleManagement` is **reserved**. Connector install and configuration happen through the connectors catalog (`manage_connectors`) and the CLI. The flag is kept as a stable config knob for a future management tool, but toggling it has no effect on the current tool set.
</Aside>
...
<Aside type="tip">
Use feature flags to lock down a production deployment. For example, disable `skillManagement` to prevent the agent from mutating skills, or disable `bundleDiscovery` to keep the agent out of the registry, while leaving the rest of the platform intact.
</Aside>
