...
- **`bundled-static`** (locked, type `static`) — the curated catalog **directory**: `src/connectors/catalog/curated/` in-image, or wherever [`NB_CURATED_CATALOG_DIR`](/config/environment/#registries) points. Every `*.yaml`/`*.yml`/`*.json` in it loads into this one registry. Cannot be disabled or removed; operators may rename it.
...
<Aside type="caution">
  The in-image catalog (`src/connectors/catalog/curated/`) ships **DCR entries only** — it exists so a fresh install has a non-empty Browse with nothing to configure. Everything else, including `auth: static` and brokered entries, belongs in the directory `NB_CURATED_CATALOG_DIR` points at.
</Aside>
