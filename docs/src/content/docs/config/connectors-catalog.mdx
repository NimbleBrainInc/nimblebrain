...
| `secretHeaders` | no, `auth: provider` only | Header name → `{ ref: credential, key }`, for a connection that must present the *workspace's* own secret as well as its own credential. Resolved per connection at that workspace's scope. Only references are accepted — a literal is refused at install, naming the header. Nothing carries the field on any other auth kind, where it is silently ignored. See [per-workspace credentials](/config/secrets/#when-the-connection-also-has-to-say-who-is-calling). |
