...
The Settings → Connectors **Browse** list is the **catalog**: one directory of [`ServerDetail`](https://github.com/modelcontextprotocol/registry) files — the shape the upstream MCP registry publishes — read, validated, and projected into the rows a workspace can install from.
...
[`NB_CURATED_CATALOG_DIR`](/config/environment/) names a directory. Every `*.yaml`, `*.yml`, and `*.json` in it is read in sorted filename order and aggregated into one catalog; unset, the runtime falls back to the minimal DCR-only example baked into the image so a fresh install is not empty.
...
<Aside type="note">
  **Filling the directory is not the runtime's job.** A GitOps ConfigMap, an operator with a text editor, or a job that mirrors some upstream index all do the same thing: write `ServerDetail` files. That is the whole extension point — pointing the platform at a second source of connectors means writing files into this directory, not configuring the runtime.
</Aside>

## Adding a new OAuth connector

Add the entry to your deployment's catalog directory — the one [`NB_CURATED_CATALOG_DIR`](/config/environment/#connectors-catalog) points at. (`src/connectors/catalog/curated/example.yaml` is the in-image example: DCR-only, handy for local dev, and replaced wholesale in any real deployment.) Either way the file is a list of upstream `ServerDetail` records; platform-specific fields live under the reverse-DNS extension key `_meta["ai.nimblebrain/connector"]`.
...
- [Environment Variables](/config/environment/) — `NB_PUBLIC_ORIGIN`, `NB_CURATED_CATALOG_DIR`.
