...
The `url` is a placeholder — the install replaces it with the brokered session URL. Unlike `auth: composio`, there is no per-connector env var: a Smithery qualified name is global, so the entry is complete on its own and only the platform-wide `SMITHERY_API_KEY` + `SMITHERY_NAMESPACE` are deployment state. There is no tool allowlist either; the connection exposes the server's own surface.
