...
- **`auth: smithery`** — the connection is brokered and hosted by [Smithery](https://smithery.ai). The catalog names a Smithery *registry qualified name* rather than an endpoint; the platform asks Smithery's Connect API for a hosted MCP session at install, and Smithery holds any upstream credential write-only. Useful for servers already published to the Smithery registry.
- **`auth: provider`** — a platform-managed connector whose credential the runtime mints itself against the fleet authorizer. Not something you author for a third-party service.
...
Brokered entries (`auth: composio`, `auth: smithery`) go in the directory `NB_CURATED_CATALOG_DIR` points at, **not** in `example.yaml` — every entry shipped in-image must install with zero operator setup, and a brokered one needs its provider configured first.
...
| `auth` | required for `remotes[]` | `"dcr"`, `"static"`, `"composio"`, `"smithery"`, or `"provider"`. Defaults to `dcr`. |
