...
It reports every entry that would be dropped — at either stage, source or directory boundary — and exits non-zero if there are any. Output is the same wording the runtime logs, so it doubles as the string to grep for when confirming a fix landed:
