...
The provider's **settings** live in `nimblebrain.json` under [`connectors.providers.composio`](/config/nimblebrain-json/#connectorsproviderscomposio). The three variables marked **fallback** below apply **per field** — each is read only when the block leaves that setting out, so declaring one setting never disturbs another.
...
The broker credential below is a fallback for [`connectors.providers.smithery`](/config/nimblebrain-json/#connectorsproviderssmithery) — including the API key, because an installed connector's transport names the `smithery` credential provider rather than an env var.
