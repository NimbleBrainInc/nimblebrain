...
The provider's **settings** are declarable in `nimblebrain.json` under [`connectors.providers.composio`](/config/nimblebrain-json/#connectorsproviderscomposio). The two variables marked **fallback** below apply **only when that block is absent** — declare the block and setting them alongside it is refused at startup.
