...
| `COMPOSIO_<TOOLKIT>_AUTH_CONFIG_ID` | Legacy | — | Superseded by [`connectors.providers.composio.authConfigs`](/deploy/composio/#3-create-one-auth-config-per-toolkit-in-composio), which keys the `ac_…` id by toolkit slug. Read only when the toolkit has no declared entry and its catalog entry still names a variable via `composio.authConfigEnv`; both go away together. |
