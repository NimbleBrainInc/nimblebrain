...
| `GREENPT_API_KEY` | For GreenPT models | — | GreenPT API key. Also settable via `providers.greenpt.apiKey`. |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `NB_WORK_DIR` | No | `~/.nimblebrain` | Working directory for runtime state (conversations, logs, workspaces). Does **not** drive config-file discovery — see [config resolution](/config/nimblebrain-json/#config-resolution). |
...
| `NB_MAX_PARALLEL_TOOL_CALLS_PER_SOURCE` | No | `6` | Most tool calls the agent dispatches to any **one** source at a time. A turn touching several connectors still fans out across them — only the depth against a single source is capped — so this bounds how loud the runtime can be toward one server, not overall throughput. Raise it for fast local servers; lower it for a fragile upstream. Zero, negative, fractional-below-1, and unparseable values fall back to the default: removing the bound is never the intended reading. Read once at startup. Scoped per source **per agent run**, which is a ceiling rather than a guarantee: concurrent runs each get their own budget, so the worst case against one server is roughly `concurrent runs x this value`, not this value. Size it with that multiple in mind. |
| `NB_CONNECTOR_START_CONCURRENCY` | No | `4` | How many connectors the boot loop starts in parallel, across every workspace. The default keeps peak memory and CPU bounded on a small pod while capturing most of the serial→parallel win; raise it when a deployment has many fast connectors and boot time matters, set it to `1` for a sequential start. Values below 1 and unparseable values fall back to the default. Read once at startup. |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `NB_CURATED_CATALOG_DIR` | No | in-image example | The connector catalog — a directory of `ServerDetail` YAML/JSON files (the intended use, hence the name), or a single such file. Every `*.yaml`/`*.yml`/`*.json` in it is read in sorted filename order and re-read on each lookup, so editing a mounted ConfigMap takes effect without a restart. Replaces `src/connectors/catalog/curated/` wholesale. This is where `auth: static` and brokered (`composio`, `smithery`) entries belong; the in-image example is DCR-only. See [Connectors Catalog](/config/connectors-catalog/). |
...
| `NB_INTERNAL_TOKEN` | No | — | Shared secret for service-to-service calls. |

<Aside type="caution">
  `NB_API_KEY` is **not** an authentication gate. It does not enable, configure, or bypass auth. To configure auth, use [`instance.json`](/config/instance-json/).
</Aside>

### Composio aggregator

Required only when the connectors catalog contains any `auth: composio` entries (Gmail, Outlook, …). See the [Composio operator guide](/gateways/composio/) for end-to-end setup.

The provider's **settings** live in `nimblebrain.json` under [`connectors.providers.composio`](/config/nimblebrain-json/#connectorsproviderscomposio).

`COMPOSIO_API_KEY` is the one value with two sources — it is also declarable as `connectors.providers.composio.apiKey`, though the environment remains the better posture for a secret ([why](/config/nimblebrain-json/#the-broker-credential)). `NB_TENANT_ID` is env-only.
...
`SMITHERY_API_KEY` is a fallback for [`connectors.providers.smithery.apiKey`](/config/nimblebrain-json/#connectorsproviderssmithery), because an installed connector's transport names the `smithery` credential provider rather than an env var. Every other Smithery setting lives in that block only.
...
The working directory contains `workspaces/<wsId>/` (which holds each workspace's conversations, files, and automations), `users/<userId>/`, `logs/`, and `cache/`.

`NB_WORK_DIR` sets the `workDir` runtime value (where state lives). It does **not** select which `nimblebrain.json` is loaded — config-file discovery is driven by `--config`, a project-local `.nimblebrain/`, and the command's default workdir. See [config resolution](/config/nimblebrain-json/#config-resolution).
