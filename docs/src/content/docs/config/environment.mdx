...
`COMPOSIO_API_KEY` falls back the same way — it is declarable as `connectors.providers.composio.apiKey`, though the environment remains the better posture for a secret ([why](/config/nimblebrain-json/#the-broker-credential)). `COMPOSIO_<TOOLKIT>_AUTH_CONFIG_ID` and `NB_TENANT_ID` are env-only.
