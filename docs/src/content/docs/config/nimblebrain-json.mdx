...
The key is the catalog entry's `composio.toolkit` — an identifier that entry already carries, so declaring an id introduces no string that has to match anything elsewhere.
