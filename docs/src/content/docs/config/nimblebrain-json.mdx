...
| Google Gemini 3 and later | `thinkingConfig.thinkingLevel`. Also tops out at `high` |
| Google Gemini 2.5 | `thinkingConfig.thinkingBudget`, sized from the tier |
...
`off` is not universally enforceable. Anthropic's adaptive-only models have no
"do not reason" state; on OpenAI the option is omitted rather than sending
`reasoningEffort: "none"`, which is documented as GPT-5.1-only and an error
elsewhere, so those models fall back to their own default. Gemini 2.5 takes a
zero budget and Gemini 3 takes `minimal`.
