...
<Aside type="note">
  **A secret in a mounted config file is still your call.** Declaring `apiKey` here works, but `COMPOSIO_API_KEY` sourced from a secret store into the pod environment remains the better posture for most deployments — `nimblebrain.json` is operator-managed and overwritten on every deploy. Use the block for `baseUrl` and `monitorEnabled`, and reach for `apiKey` when a single-tenant or local setup makes one config file simpler than two mechanisms.
</Aside>
