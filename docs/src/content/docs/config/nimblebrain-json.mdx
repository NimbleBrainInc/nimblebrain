...
| `thinking` | `"off" \| "adaptive" \| "enabled"` | — | Extended-thinking mode for reasoning-capable models. `off`: requests no reasoning — **not currently enforceable on adaptive-only Anthropic models, which includes the default** ([#738](https://github.com/NimbleBrainInc/nimblebrain/issues/738)): the AI SDK adapter accepts the flag and then never sends it, so the model applies its own default, and Opus 5's default is to reason. `adaptive`: model decides per call. `enabled`: always reason (cap with `thinkingBudgetTokens`). Typically set via the admin `set_model_config` tool, which writes it to `nimblebrain.overrides.json`. |
