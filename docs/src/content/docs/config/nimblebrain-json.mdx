...
|---|---|
| Anthropic — Opus 4.7/4.8, Sonnet 5, Opus 5 | `thinking: {type: "adaptive"}` + `output_config.effort` |
| Anthropic — up to Sonnet 4.6 | `thinking: {type: "enabled"}` + a `budgetTokens` sized from the tier |
| OpenAI, and Nebius-hosted open-weight models (DeepSeek, Kimi, …) | `reasoningEffort`. Its ladder stops at `xhigh`, so `max` clamps to it |
| Google Gemini | `thinkingConfig.thinkingBudget`, sized from the tier |
...
Set a budget with `thinkingBudgetTokens` only when you want a hard token cap on
a provider that meters tokens. It takes precedence over `thinkingEffort` there;
on the effort-shaped providers it cannot be expressed and the default tier
applies.
