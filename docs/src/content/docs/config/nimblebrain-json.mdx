...
The `models` object declares three role-based slots. Each value is a `provider:model-id` string (e.g. `"anthropic:claude-opus-5"`).
...
|------|------------------------|
| `default` | `defaultModel`, else `anthropic:claude-opus-5` |
| `reasoning` | `defaultModel`, else `anthropic:claude-opus-5` |
| `fast` | `defaultModel`, else `anthropic:claude-haiku-4-5-20251001` |

`fast` is deliberately asymmetric: with nothing configured it lands on a small model rather than following `default`. The slot backs short, single-shot calls made outside the agent loop — titling, history compaction, the briefing — on output budgets sized to their answer. Two reasons that matters:
