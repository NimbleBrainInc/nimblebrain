...
<Aside type="caution">
  Don't put a **small-context** model in the `fast` slot. A `fast` request (via `nb__delegate` or a `"model": "fast"` override) runs the **full tool surface**, and the message budget is `context − systemPrompt − toolSchemas − maxOutputTokens − safetyMargin`. On a connector-heavy workspace, a model with a ~40k window can be left with almost no room for conversation history — and once that budget hits zero, only the latest message is kept. Choose the `fast` model for generous context (≥128k), not just low latency or price.
</Aside>
