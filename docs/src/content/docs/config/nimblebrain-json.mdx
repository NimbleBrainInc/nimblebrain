...
The key is the catalog entry's `composio.toolkit` — an identifier that entry already carries, rather than a third name invented to carry the value. It is still matched exactly, so a mistyped key resolves to nothing; the runtime checks the whole set at boot and logs any key matching no catalog toolkit, along with any toolkit left with no id at all.
