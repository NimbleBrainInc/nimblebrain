...
| OpenAI, and Nebius-hosted open-weight models (Qwen, GLM, gpt-oss) | `reasoningEffort`. Its usable range stops at `high`, so `xhigh` and `max` both land there. On OpenAI, acceptance is also per model — see below |
...
```json
{
  ...
  "models": {
    ...
    "reasoning": "nebius:Qwen/Qwen3-Next-80B-A3B-Thinking"
  }
}
```
