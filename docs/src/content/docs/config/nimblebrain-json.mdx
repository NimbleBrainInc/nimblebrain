...
- **`workspace.json`** — per-workspace settings (connectors, skill directories, optional model + identity overrides). See [`workspace.json`](/config/workspace-json).

Top-level `skillDirs`, `home`, and `preferences` entries in `nimblebrain.json` are stripped at load — they belong in `workspace.json` now. See [workspace configuration](/config/workspace-json) for where they live. A workspace's `connectors` array belongs there too; in this file the name `connectors` is the [provider and gateway block](#connectors).
...
<Aside type="caution">
  `NB_WORK_DIR` does **not** drive config-file discovery. It sets the `workDir` runtime value (where conversations, logs, and workspaces are stored), but the config file is located by the order above. To point at a specific config file, use `--config`.
</Aside>
...
| `modelPolicy` | `object` | — | Which models this organization permits. See [modelPolicy](#modelpolicy). |
...
| `notifications` | `object` | — | Pacing for the notification outbox poll. See [notifications](#notifications). |
| `files` | `object` | — | File upload limits. See [files](#files). |
| `allowInsecureRemotes` | `boolean` | `false` | Allow HTTP (non-TLS) remote connector connections. Dev only. |
...
<Aside type="note">
  **Workspace-owned fields** — `skillDirs`, `home`, `preferences` — are stripped if placed at the top level. They live in [`workspace.json`](/config/workspace-json). A top-level `skills` array is a **legacy** field and is stripped too; skill directories belong in `workspace.json` as `skillDirs`. A workspace-shaped `connectors` **array** is rejected outright, because in this file that name is the [provider and gateway block](#connectors) and takes an object.
...
Google and OpenAI support is **per model, not per generation**, and is read
from hand-maintained tables measured against the live APIs.
...
- **Gemini** — 2.5 Flash takes a zero budget; 2.5 Pro cannot disable thinking.
  On Gemini 3 it depends on the model: those offering a `minimal` level get it,
  and those that don't (both Pro entries) get nothing rather than the next tier
  up, which would answer "don't reason" with an instruction to reason.
...
A workspace can override individual slots via `workspace.json → models`. When a per-request model specifies a slot name (e.g. `"model": "fast"`), it's resolved against these slots. `"alias:fast"` is the same reference written explicitly; both spellings resolve identically.
...
A conversation is bound to a model when it is created, and that binding does not
...
current slots: title generation and history compaction take `fast`.
...
Conversations created before this behavior existed carry no binding and resolve
from the current slot on every turn, as they did before.

<Aside type="caution">
  Don't put a **small-context** model in the `fast` slot. A `fast` request (via a `"model": "fast"` override) carries the **same tool surface as a normal request** — it isn't slimmed down for being "fast" — so its message budget is capped by the model's headroom: `context − systemPrompt − toolSchemas − maxOutputTokens − safetyMargin` (a lower `maxInputTokens` caps it further). On a connector-heavy workspace, a model with a ~40k window can be left with almost no room for conversation history — in the worst case the window keeps only the anchor (the first turn) and the last atomic group, dropping the middle of the thread, so the agent loses mid-conversation context. Worse, a single large tool result (a file read or web fetch) can push even that minimal window past the model's context, so the provider rejects the request outright. Prefer a `fast` model with generous context (≥128k).

  The history folds want the same headroom for a different reason. A fold summarizes a conversation grown against the **main** model's window, so a `fast` model with a smaller one has less room to read what it is summarizing — the call is bounded to the summarizer's own context to keep it from overflowing, and a small window buys a worse summary or none at all.
</Aside>

## modelPolicy

Which models this organization permits. Absent — or present with an empty
...
Policy can only **subtract**. It narrows what the configured providers can
already reach — naming a model whose provider has no key does not make it
reachable, and naming a non-chat model does not make it selectable.

**This file is the only place it is set.** There is no runtime surface for it —
the allowlist is deployment configuration, so it lives in version control with
the rest of the tenant's setup and a redeploy always restores it.

Every slot in [`models`](#models) should name an allowed model. Nothing stops
...
Once the file is loaded the runtime does hold the line — repointing a slot at a
forbidden model, or clearing one so it falls back to a forbidden model, is
refused. A *user's* saved model preference needs no such care: one outside the
list falls back to the configured default on their next turn.
...
Each provider key is optional. If a provider's `apiKey` is omitted, the runtime falls back to that provider's standard env var (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY`, `GREENPT_API_KEY`, `NEBIUS_API_KEY`, `XAI_API_KEY`).
...
GreenPT is a European AI provider with an OpenAI-compatible API, optimized infrastructure, and data centers powered by 100% renewable energy. Configure it with the `greenpt:` prefix:
...
```json
{
  ...
  "models": {
    "default": "greenpt:glm-5.2",
    "fast": "greenpt:kimi-k2.7-code",
    "reasoning": "greenpt:kimi-k3"
  }
}
```

The key falls back to `GREENPT_API_KEY`. Like `nebius` below, this key is **required when `greenpt` is configured** — the runtime fails to start if neither `providers.greenpt.apiKey` nor `GREENPT_API_KEY` is set, because the GreenPT adapter reads no environment variable of its own and a keyless provider has nothing to authenticate with. Override the endpoint with `baseURL` (default `https://api.greenpt.ai/v1`). The catalog includes the flagship `glm-5.2`, coding-focused `kimi-k2.7-code`, and long-context `kimi-k3`; other chat model IDs still route as free text. Use `GET https://api.greenpt.ai/v1/models` with your API key for the current model list.

GreenPT turns are sent without a reasoning-effort tier, so each model reasons at its own default and `thinkingEffort` has no effect here. GreenPT documents an effort ladder for `kimi-k3` only and publishes no reasoning parameter for the other catalog models, so there is no tier known to be accepted across them — sending an unaccepted value would fail the call outright, which is the worse trade.
...
| `authConfigs` | `Record<string, string>` | `{}` | Per-toolkit auth-config ids (`ac_…`), keyed by the toolkit slug the catalog entry names. See [Per-toolkit auth configs](#per-toolkit-auth-configs). |
...
Every `auth: composio` connector needs an auth config created in the Composio dashboard ([how](/gateways/composio/#3-create-one-auth-config-per-toolkit-in-composio)). Its `ac_…` id varies per Composio account, so it cannot live in the catalog, which is shared across deployments.
...
It is declarable because nothing persisted points at where it lives. A connector installed through Composio records `auth: { type: "provider", provider: "composio" }` in `workspace.json` — it names the credential it needs, not a place to find it — so the key never sits at rest and resolution stays the provider's own business. Connectors installed before that change carry a `${COMPOSIO_API_KEY}` environment reference in their `auth` block and are mapped forward when they start, so both resolve identically with no operator action. The key itself may be a [credential reference](/config/secrets/) rather than a literal.
...
An app can declare an **outbox** — one resource carrying facts it recorded that
nobody asked for, such as a domain going active or a reply landing. The runtime
reads each declared outbox on a schedule and writes what it finds to the
workspace's notification inbox. What an outbox carries is the app's decision and
who may declare one is yours; what the poll *costs* is the runtime's, and this
block is where you move it.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `poll.intervalMs` | `integer` (ms) | `60000` | Base cadence per `(workspace, connector)`. Floored at the fixed 15,000 ms poll floor. |
| `poll.maxIntervalMs` | `integer` (ms) | `300000` | Backoff ceiling — the slowest an idle outbox is read, and the interval an open circuit breaker half-opens on. Never resolves below `intervalMs`. |
| `poll.maxEvents` | `integer` | `100` | Events requested per read. Capped at 1000. |
| `poll.budgetPerMinute` | `integer` | `10` | Polls allowed per workspace per minute, across every connector in it. |

Nothing here is required: a deployment that sets no block polls at these
defaults, and one where no app declares an outbox polls nothing at all.

### The cadence is adaptive, and an app may recommend one

Each outbox is read on its own schedule, not on a fleet-wide tick:

- Consecutive reads that return no events **double** the interval, up to
  `maxIntervalMs`. A read that returns events snaps straight back to
  `intervalMs`.
- An answer carrying `nextPollMs` is honoured, clamped between a fixed 15-second
  floor and `maxIntervalMs`. An app watching something that is about to change
  asks to be read sooner this way; one that is rate-limited upstream asks to be
  read less.
- An answer carrying `hasMore: true` means the batch was cut at `maxEvents`, and
  the runtime reads again immediately — out of the same budget, because it is
  the same cost.
- An outbox that fails three reads in a row is read once per `maxIntervalMs`
  until it answers, rather than on every tick.

The 15-second floor is not configurable. It bounds how much of a tenant's
request budget a standing background loop can consume, and an app that wants to
be read faster than that wants a push rather than a poll.

### The budget, and why the default is what it is

`budgetPerMinute` is a per-workspace allowance, not a global one. The default is
derived rather than picked: the poll may take about a quarter of a tenant's
request rate, and one poll costs up to three requests when it has to re-establish
an idle-closed connection first — so the allowance buys a third as many polls as
requests.

When a workspace's allowance runs out mid-pass, the connectors it did not reach
are deferred to the next tick and the pass **resumes with them**. A workspace
with more outboxes than budget reads all of them, more slowly — it never reads
the first few forever.

<Aside type="caution">
  The poller runs in every replica, so `replicas: 1` is the supported topology —
  the same prerequisite the connection revalidator carries. Two replicas would
  read the same outbox twice and race the same stored position. See
  [Observability](/deploy/observability/#background-loops).
</Aside>
...
NimbleBrain collects anonymized, aggregate usage data with no PII — no connector names, tool names, error messages, or file paths are sent.
...
- **Workspace-owned and legacy keys** (`skillDirs`, `home`, `preferences`, and the legacy `skills`) warn as unknown keys and are stripped before the runtime config is built.
...
This uses all defaults: Anthropic provider (key from `ANTHROPIC_API_KEY`), default model, no connectors installed (platform capabilities are built in), in-memory conversation storage, and structured logging enabled.
