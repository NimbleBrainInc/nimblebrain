...
- **Block present** — the block wins for **both** settings and those two are ignored, with one startup warning naming the ones that were set.

Precedence is whole-block, not per-field: a block that sets only `monitor` also takes the default `baseUrl`, rather than inheriting `COMPOSIO_API_BASE_URL`. `COMPOSIO_API_KEY` is unaffected either way.
