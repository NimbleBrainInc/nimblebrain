...
| `maxOutputTokens` | `integer` | the model's catalog `limits.output` | Max output tokens per LLM call. Unset means the resolved model's own ceiling (128,000 on Opus 5), not a fixed number — `16384` applies only when the model isn't in the catalog. |
| `maxToolResultSize` | `integer` | `1000000` | Max characters for a single tool result. `0` disables the cap. |
| `thinking` | `"off" \| "adaptive" \| "enabled"` | — | Extended-thinking mode for reasoning-capable models. `off`: requests no reasoning — **not currently enforceable on adaptive-only Anthropic models, which includes the default** ([#738](https://github.com/NimbleBrainInc/nimblebrain/issues/738)): the AI SDK adapter accepts the flag and then never sends it, so the model applies its own default, and Opus 5's default is to reason. `adaptive`: model decides per call. `enabled`: request reasoning on every turn (cap with `thinkingBudgetTokens`) — **on adaptive-only models with neither `maxOutputTokens` nor `thinkingBudgetTokens` set, which is the out-of-box shape for `default` and `reasoning`, this behaves as `adaptive`**: those models cannot carry a budget, and deriving one from the model's catalog ceiling would request the deepest reasoning tier on every call. Typically set via the admin `set_model_config` tool, which writes it to `nimblebrain.overrides.json`. |
| `thinkingBudgetTokens` | `integer` | — | Token budget when `thinking: "enabled"`. Counts toward `maxOutputTokens`. Also honored for `adaptive`: on an adaptive-only model the budget selects an `output_config.effort` tier rather than a hard cap. Anthropic requires a minimum of 1,024. |
...
The `models` object declares three role-based slots. Each value is a `provider:model-id` string (e.g. `"anthropic:claude-sonnet-5"`).
...
### Fallbacks when a slot is unset

Every slot is optional. An unset slot resolves in this order:

| Slot | Unset → falls back to |
|------|------------------------|
| `default` | `defaultModel`, else `anthropic:claude-sonnet-5` |
| `reasoning` | `defaultModel`, else `anthropic:claude-opus-5` |
| `fast` | `defaultModel`, else `anthropic:claude-haiku-4-5-20251001` |

`fast` and `reasoning` are deliberately asymmetric: with nothing configured each lands on a model suited to its own workload rather than following `default`. `reasoning` steps up a tier for work that justifies it — deep analysis, planning, long-horizon tasks. `fast` steps down. The slot backs short, single-shot calls made outside the agent loop — titling, history compaction, the briefing — on output budgets sized to their answer. Two reasons that matters:

- **Cost.** Titling runs on essentially every conversation, so this is the platform's highest-frequency model call. A frontier model here is its largest avoidable spend.
- **Correctness.** `max_tokens` bounds thinking *and* visible text together. On a model that reasons when not asked to, a small budget can go entirely to a reasoning trace and return no content — titling then falls back to a truncated first message, silently.

An explicit `defaultModel` still governs every unset slot, `fast` and `reasoning` included. That keeps single-provider deployments working: if `fast` skipped straight to a Claude model, an installation configured with only Google or only Nebius credentials would resolve the slot to a model it cannot call. If you set `defaultModel` to a frontier model and want the cheap-slot behavior, set `models.fast` explicitly.
