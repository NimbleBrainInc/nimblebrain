...
Every slot in [`models`](#models) should name an allowed model. Setting a policy
that excludes one **through the settings surface is rejected**; written here by
hand it is not — the runtime logs an error naming the slot at startup and then
runs, because a deployment that boots with a warning it can act on beats one
that will not start. The effect until you fix it: turns resolve to that model
while the picker excludes it.

A *user's* saved model preference needs no such care: one outside the list
falls back to the configured default on their next turn.
