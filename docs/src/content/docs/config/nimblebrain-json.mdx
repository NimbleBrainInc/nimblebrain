...
A toolkit with no entry here simply has no auth config: installing it fails with a message naming the toolkit and this config path. That is a deliberate non-error — the catalog is a menu, and a deployment wires the toolkits it wants.
