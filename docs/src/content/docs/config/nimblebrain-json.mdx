...
| OpenAI, and Nebius-hosted open-weight models (DeepSeek, Qwen, gpt-oss) | `reasoningEffort`. Its usable range stops at `high`, so `xhigh` and `max` both land there. On OpenAI, acceptance is also per model — see below |
...
On OpenAI, most models take all three tiers, but the `-pro` line does not:
`gpt-5-pro` accepts only `high`, and `gpt-5.2-pro`, `gpt-5.4-pro` and
`gpt-5.5-pro` reject `low`. `gpt-5.2-chat-latest` accepts only `medium` —
bounded from above as well as below. An OpenAI model with no entry is assumed
to take all three, the opposite of the Google default, because nearly every
model does and the adapter already skips non-reasoning ones.
...
A toolkit with no entry falls back to the environment variable named by its catalog entry's legacy `composio.authConfigEnv` — the one field here whose fallback is per-key rather than per-field. The runtime logs a deprecation warning naming the toolkit whenever that fallback is the value's actual source, so the toolkits still to migrate are visible in your logs. Both the fallback and the field will be removed ([#789](https://github.com/NimbleBrainInc/nimblebrain/issues/789)).
...
| `authConfigs` | per-toolkit — see [Per-toolkit auth configs](#per-toolkit-auth-configs) |
