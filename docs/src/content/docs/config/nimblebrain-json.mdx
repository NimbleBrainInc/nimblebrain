...
A toolkit with no entry falls back to the environment variable named by its catalog entry's legacy `composio.authConfigEnv`. That fallback carries a running deployment through the move and will be removed with the field.
