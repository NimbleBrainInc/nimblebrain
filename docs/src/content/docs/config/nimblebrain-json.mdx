...
Managed-connector providers. A **managed-connector provider** is a third party that brokers a connector's auth (OAuth or API key) *and* hosts the MCP session on your tenant's behalf. Composio is the one shipped today.

A provider is registered — and its callback routes, liveness probe, and vendor SDK reached — **only when its broker credential is present**. A deployment without one carries no provider: the `/v1/composio-auth/*` routes are not mounted, no revalidation sweep runs, and the vendor SDK is never loaded.
...
| `monitor.enabled` | `boolean` | `true` | Run Composio's arm of the connection-revalidator probe, which detects connections whose downstream vendor account lapsed while the platform key stayed valid. |
...
**The Composio API key is not declared here.** It is read from `COMPOSIO_API_KEY` in the process environment, and the config block does not supersede it.

This is deliberate, and it is about *where persisted state points*. A connector installed through Composio persists its transport credential in `workspace.json` as the secret reference `` `${COMPOSIO_API_KEY}` `` — the key itself never sits at rest — and resolves it from the process environment each time the connection starts. That reference is a durable pointer into the environment namespace, so a credential declared anywhere else would leave already-installed connectors authenticating with an empty header.

The fix is for persisted state to name a *credential provider* rather than an environment variable, at which point resolution becomes the provider's private business and the credential can join the block. Until then it stays where persisted state already points. Keep it in your secret store → pod environment, which is the better posture for a secret regardless.

### Environment fallback and precedence

The `COMPOSIO_*` settings variables ([Environment Variables](/config/environment)) still work, as a **fallback**:

- **Block omitted** — `COMPOSIO_API_BASE_URL` and `COMPOSIO_MONITOR_ENABLED` hydrate the provider's settings. Nothing changes for an existing deployment.
- **Block present** — the block owns **both** settings, and setting either env var alongside it is **refused at startup** with an error naming them.

Precedence is whole-block, not per-field: a block that sets only `monitor` also takes the default `baseUrl`, rather than inheriting `COMPOSIO_API_BASE_URL`. That makes the rule lossy, which is why the conflict is an error rather than a warning — `monitor.enabled` is a kill switch, and a block added for an unrelated reason would otherwise default a deliberately-disabled probe back on, with the consequence (connectors flipping to `reauth_required`) surfacing long after any log line. Pick one source per setting.

`COMPOSIO_API_KEY` is unaffected either way — it is the credential, not a setting, and is never superseded.
