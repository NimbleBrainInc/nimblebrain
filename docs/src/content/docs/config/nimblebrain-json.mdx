...
Every slot in [`models`](#models) must name an allowed model. Setting a policy
that excludes one is rejected rather than accepted and left to fail at the next
turn. A *user's* saved model preference needs no such care: one outside the
