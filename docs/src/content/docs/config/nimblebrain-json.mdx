...
- **`workspace.json`** — per-workspace settings (bundles, skill directories, optional model + identity overrides). See [`workspace.json`](/config/workspace-json).

Top-level `bundles`, `skillDirs`, `home`, and `preferences` entries in `nimblebrain.json` are silently stripped at load — they belong in `workspace.json` now. See [workspace configuration](/config/workspace-json) for where they live.
...
- **Workspace-owned and legacy keys** (`bundles`, `skillDirs`, `home`, `preferences`, `noDefaultBundles`, and the legacy `skills`) are silently stripped — no warning.
