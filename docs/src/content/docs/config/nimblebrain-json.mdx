...
Every `auth: composio` connector needs an auth config created in the Composio dashboard ([how](/deploy/composio/#3-create-one-auth-config-per-toolkit-in-composio)). Its `ac_…` id varies per Composio account, so it cannot live in the catalog, which is shared across deployments.
