...
|---|---|---|
| `workspace` (named volume) | `/data` | Working directory (`NB_WORK_DIR`) — conversations, logs, installed connectors, and `nimblebrain.json` |
...
| `GREENPT_API_KEY` | No | GreenPT API key (for the GreenPT model provider) |
...
| Node.js + npm | 24 | Builds the in-image platform app UIs (home, conversations, files, automations) |
