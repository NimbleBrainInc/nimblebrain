...
- It is unauthenticated. It carries no per-user data and is meant to be reachable only inside the cluster.
...
The platform serves a liveness/readiness endpoint at `GET /v1/health`. It answers `200` with a fixed body once the server is serving:
...
| Whether connectors are up | The `nb_connector_unhealthy` gauge on [`/metrics`](#prometheus-metrics): `1` for each connector that is currently down, and no series for a healthy one |
