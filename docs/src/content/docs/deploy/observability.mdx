...
The platform serves a liveness/readiness endpoint at `GET /v1/health`. It is unauthenticated and returns the build identity plus the state of each installed connector:
...
```json
{
  ...
  "buildSha": "abc1234",
  "connectors": [
    { "name": "example", "state": "healthy" },
    { "name": "example", "state": "restarting" }
  ]
}
```
...
| `connectors` | One entry per running connector **instance** and its lifecycle state. Entries are **not unique by name**: a connector installed in several workspaces is monitored separately in each, so it appears once per workspace. Don't build a `name → state` map from this — later entries would silently overwrite earlier ones. |
...
<Aside type="note">
There is no `uptime` field. Use `version` / `buildSha` to confirm which build is live and `connectors[].state` to confirm connectors started cleanly.
</Aside>
