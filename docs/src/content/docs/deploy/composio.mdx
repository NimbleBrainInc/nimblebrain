...
The key must match the catalog entry's `composio.toolkit` exactly. At boot the runtime logs any key matching no catalog toolkit, any toolkit with no id at all, and any toolkit still resolving from the deprecated environment fallback — so check your startup logs after changing this block.

A toolkit with no entry here falls back to the environment variable named by its catalog entry's legacy `composio.authConfigEnv` (`COMPOSIO_GMAIL_AUTH_CONFIG_ID` and friends). That fallback exists so a running deployment keeps working while its ids move into the block. Whenever it is the value's actual source the runtime logs a deprecation warning naming the toolkit — grep your logs for `[composio]` to see which toolkits are still to migrate. Both the fallback and the field will be removed ([#789](https://github.com/NimbleBrainInc/nimblebrain/issues/789)).
...
Precedence is per field: a field declared here wins, a field left out reads its env var (`COMPOSIO_API_KEY`, `COMPOSIO_API_BASE_URL`, `COMPOSIO_MONITOR_ENABLED`). The revalidator's sweep cadence is provider-agnostic and stays on `NB_CONNECTION_REVALIDATE_INTERVAL_SECONDS`. `apiKey` is declarable too, though a secret store → pod environment remains the better posture — see [the broker credential](/config/nimblebrain-json/#the-broker-credential). See [`connectors`](/config/nimblebrain-json/#connectors) for the full surface.
