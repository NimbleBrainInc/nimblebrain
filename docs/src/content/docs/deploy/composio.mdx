...
A toolkit with no entry here falls back to the environment variable named by its catalog entry's legacy `composio.authConfigEnv` (`COMPOSIO_GMAIL_AUTH_CONFIG_ID` and friends). That fallback exists so a running deployment keeps working while its ids move into the block, and will be removed — declare them here.
