...
Precedence is whole-block: with the block present, `COMPOSIO_API_BASE_URL` and `COMPOSIO_MONITOR_ENABLED` are ignored (one startup warning names them). The revalidator's sweep cadence is provider-agnostic and stays on `NB_CONNECTION_REVALIDATE_INTERVAL_SECONDS`. `COMPOSIO_API_KEY` is unaffected — the broker credential is always read from the environment and is what gates the provider being registered at all. See [`connectors`](/config/nimblebrain-json/#connectors) for the full surface.
...
```
[composio] integration: configured (source=env, base=https://backend.composio.dev)
```
