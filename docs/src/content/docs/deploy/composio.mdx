...
Precedence is per field: a setting declared here wins, a setting left out reads its env var (`COMPOSIO_API_BASE_URL`, `COMPOSIO_MONITOR_ENABLED`). The revalidator's sweep cadence is provider-agnostic and stays on `NB_CONNECTION_REVALIDATE_INTERVAL_SECONDS`. `COMPOSIO_API_KEY` is unaffected — the broker credential is always read from the environment and is what gates the provider being registered at all. See [`connectors`](/config/nimblebrain-json/#connectors) for the full surface.
...
The API key is always environment-sourced.
