...
The key must match the catalog entry's `composio.toolkit` exactly. At boot the runtime logs any key matching no catalog toolkit, and any toolkit still resolving from the deprecated environment fallback — so check your startup logs after changing this block.
