...
| **provider** | A connector whose credential is supplied server-side by a named credential provider — no vendor sign-in, no operator OAuth app, no per-user secret. Covers both the platform's own connectors, whose credential the runtime mints against the fleet authorizer, and a [hosted gateway](/config/connector-providers/#hosted-mcp-gateways) you hold one account key for. | Click **Install**. There is nothing to enter. | Operator — the catalog entry, plus the fleet-authorizer environment or the gateway's declared key |
