...
Connectors authenticate in one of five ways. As an end user you rarely need to care which — the **Install / Connect** button does the right thing — but it helps to know what's happening:
...
| **smithery** | The service is brokered through Smithery, a managed-connector provider that also hosts the MCP session. | Click **Install**. Servers that need no account connect immediately. One that needs sign-in or configuration reports it on the install attempt, with a link to a Smithery-hosted setup page to complete first. Requires the platform operator to have configured Smithery. |
