...
1. Resolves the named agent profile (if specified) — an unknown name falls back to the default sub-agent rather than failing
2. Determines the system prompt — from the profile, or a fixed safety preamble when no profile is used (never the task text itself)
...
<Aside type="note">
  The `task` parameter is always required. When no `agent` profile is used, the sub-agent runs with a fixed safety preamble as its system prompt — the task text is passed as the user message, never as the system prompt. Naming a profile that doesn't exist is not an error: the delegation runs with that default sub-agent and the result notes the fallback. The `agent` parameter is only offered when the workspace has profiles configured.
</Aside>
...
| `model` | string | No | Model override. Accepts a slot name (`"default"` or `"fast"`) or `provider:model-id`. Name a model directly when the agent needs a more capable one than the workspace default. |
...
The agent will call `nb__delegate` with just a `task` parameter; the sub-agent runs with the default safety preamble as its system prompt and the task as its user message.
