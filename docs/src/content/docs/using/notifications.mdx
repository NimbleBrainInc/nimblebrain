...
| **The ceiling on … is …** | The route never matched, so nothing was dispatched. Raise that source's ceiling above, or lower the route's minimum level. |
| a target and an outcome | The dispatch was refused or failed. The wording is the ledger's — see [the delivery ledger](#the-delivery-ledger) for what each outcome means. |
