...
<Aside type="caution" title="Routes are saved but not executed in this version">
The editor stores, validates and lists routes, and nothing dispatches them yet.
The notice at the top of the settings tab says so, and disappears in the version
that turns them on. The inbox and the ceilings work now.
</Aside>
...
| Field | Means |
|---|---|
| **Source** | Exactly one connector. Omit for any. |
...
Once a route has tried to deliver an item, the expanded row shows one line per
target: what it aimed at, whether it delivered, and the last error if it did
not. Items with nothing tried show nothing.
