...
whether a dot may be crossed: `*` matches within one dotted segment, `**`
matches across as many as it likes. Nothing else is a wildcard — a `.` means a
dot and not "any character". The
[full table](/using/notification-reference#event-name-globs) has every case.
...
Then it tells you what happened, in one line —
[what each answer means and what to do about it](/using/notification-troubleshooting#start-with-send-test).
...
- [Delivering to a channel](/using/notification-channels) — Slack, Teams, email,
  WhatsApp and anywhere else, with the arguments each one takes.
- [When a notification doesn't arrive](/using/notification-troubleshooting) —
  Send test, the delivery ledger, and where a notification stops.
- [Route reference](/using/notification-reference) — every field, match rule,
  placeholder and outcome, precisely.
