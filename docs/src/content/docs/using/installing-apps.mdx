...
<Tabs>
  ...
  <TabItem label="Local bundle (config)">
    ...
    After editing, restart the server. In dev mode, `bun run dev` reloads on source changes.

    <Aside type="note">
      Bundles are not managed from the CLI. Install through the Apps catalog or the agent; for local development, edit the config directly as shown above.
    </Aside>
  </TabItem>
</Tabs>
...
For workspace-scoped values or when env vars aren't a good fit, set them from workspace settings in the web UI. Stdio bundles that declare `user_config` fields surface them on the app's **Configure** page. See [Credentials](/config/credentials/) for the full picture.
