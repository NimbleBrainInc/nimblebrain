...
Two core skills are always-on for every workspace and need no setup:
...
Both ship with the platform, so they can't be edited in place — `skills__update` refuses to write a platform-vendored file. Change one by **authoring your own skill with the same name** in a scope that outranks it.
...
<Aside type="note">
  An override you author lands at `priority` 11–99; 0–10 is reserved for the platform's own core skills. That only changes where it sits in the prompt (contained user context rather than raw identity), not whether it applies.
</Aside>
...
- [Connectors](/using/connectors) — install the servers a skill's tools come from
