...
When a task matches a listed skill, the model loads it with the `skills__use` tool, which returns the skill's full body in the tool result. A skill is delivered at most once per conversation — loading one that's already in context (via `skills__use` or a connector overlay) returns a short "already loaded" note instead of a second copy.
