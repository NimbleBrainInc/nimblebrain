...
`skills__list` says which skills *exist*. To see which ones actually reached the model, and how, ask the agent to replay the skill-load ledger (`skills__loading_log`). It returns one row per load — skill, timestamp, conversation, tokens, and a `loaded_by` channel:
