...
The workspace's own data — conversations, files, automations, credentials — is then **archived**, not destroyed: the subtree moves to `archived/<wsId>/` with a marker file. From every other surface the workspace is gone the moment the delete returns.
