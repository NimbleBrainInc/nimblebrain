...
After changing `nimblebrain.json` (adding, removing, or modifying bundle entries), restart the server to pick up the changes. In development, `nb dev` restarts the API automatically on source changes. On the managed platform, configuration changes are applied for you by the platform.
