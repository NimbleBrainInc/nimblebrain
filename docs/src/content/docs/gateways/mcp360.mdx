...
`glama` then reads `GLAMA_API_KEY`, and entries naming `providerAuth.provider: glama` authenticate with it.
...
**Installing fails naming a missing API key.** The gateway is declared and no key resolved. The error names both places to set it.
