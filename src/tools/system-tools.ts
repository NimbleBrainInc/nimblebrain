...
import { createGetOutputTool, resolveOutputResource } from "./get-output.ts";
...
export async function createSystemTools(
  ...
  // Resolve `files://<id>` output refs through the `nb` source. The frontend
  // fetches a `resource_link` with the OWNING tool's app name, and
  ...
  // would 404. Present only when an output-store provider resolved (mirrors how
  // `getOutputCtx` gates `nb__get_output`); when the provider is `null` the
  // handler is omitted and no `files://` ref renders.
