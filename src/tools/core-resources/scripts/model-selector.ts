...
function render(currentModel) {
  app.innerHTML = '<input type="text" id="model-input" value="' + (currentModel || '') + '" placeholder="provider:model-id" style="width:100%;padding:4px 8px;font-size:13px;border:1px solid var(--color-border-primary,#e4e4e7);border-radius:4px;background:var(--color-background-primary,#fff);color:var(--color-text-primary,#09090b)" />';
