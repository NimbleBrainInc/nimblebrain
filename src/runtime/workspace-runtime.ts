/**
 * Workspace-aware connector lifecycle helpers.
 *
 * These functions build a process inventory from workspace definitions and
 * manage hot install/uninstall of connectors within individual workspaces.
...
import type { ConnectorRef, LocalConnectorMeta } from "../connectors/runtime/types.ts";
...
export interface ProcessInventoryEntry {
  ...
  /** Absolute path to the workspace-scoped data directory for this connector. */
  dataDir: string;
  ...
  /**
   * Set when boot-start failed for an installed URL connector. The entry survives
   * so the connector keeps its lifecycle instance and placements, but the message
   * is what stops the seeder from recording an inaccurate `running` — it seeds
   * `dead` with this as `lastError` instead. Absent means the connector either
 ...
 * workspace-scoped via `resolveConnectorDataDirForRef`.
 */
export function buildProcessInventory(
  ...
  for (const ws of workspaces) {
    ...
    for (const connector of ws.connectors) {
      ...
      const dataDir = resolveConnectorDataDirForRef(workDir, ws.id, connector);
...
// ---------------------------------------------------------------------------
// Workspace-scoped connector startup
// ---------------------------------------------------------------------------

/** URL connector variant of `ConnectorRef` (the remote-connector shape). */
type UrlConnectorRef = ConnectorRef;

/**
 * Whether a boot-time URL connector already has credentials to auto-start with.
 *
 * A brokered connector carries static auth but may STILL need a per-owner connect,
 ...
 * OAuth connectors gate on persisted tokens.
 */
async function urlConnectorHasBootAuth(
  managedConnectors: ManagedConnectorRegistry,
  connector: UrlConnectorRef,
  wsId: string,
  serverName: string,
  workDir: string,
): Promise<boolean> {
...
/**
 * Inventory entry for an installed URL connector that is NOT running after the boot
 ...
 * facts. Only surviving entries reach `seedWorkspaceConnectorInstances`, so a
 * dropped entry costs the connector its lifecycle instance AND its placements: the
 ...
 * value only to keep the `LocalConnectorMeta` shape whole; it is never the source.
 */
function unstartedUrlConnectorEntry(
...
/**
 * Start all connectors across all workspaces, returning a per-workspace ToolRegistry.
 *
 * Reads workspaces from the store, builds the process inventory,
 * and spawns one connector process per entry. Each workspace gets its own
 * ToolRegistry containing:
 * - Platform sources (in-process MCP — conversations, files, home, etc.)
 * - System source (`nb`, in-process MCP)
 * - Workspace-specific connector sources (subprocess or remote MCP)
 *
 * Returns a Map<wsId, ToolRegistry> plus the inventory entries for lifecycle seeding.
 */
export async function startWorkspaceConnectors(
  ...
  opts?: {
     ...
     * `startConnectorSource` so re-spawned connectors register inbound
     ...
     * until the next reinstall. Runtime threads `Runtime.getConnectorMcpDeps`
     * through here.
     */
    getConnectorMcpDeps?: (wsId: string) => ConnectorMcpDeps | undefined;
    /**
     * Fired when a boot-started URL connector's connection later loses its
     ...
     * `ConnectorLifecycleManager` doesn't exist yet at boot — this fires LATER
    ...
    /**
     * The configured brokered providers. A brokered connector's boot readiness is
     * its provider's to answer (`hasConnection`); with none passed, every
     * connector falls back to the generic static-auth / persisted-token check.
     */
    managedConnectors?: ManagedConnectorRegistry;
  },
  ...
  // Also create registries for workspaces with no connectors
  for (const ws of workspaces) {
  ...
  // connector subprocess; concurrent fan-out lets k8s/CPU overlap them. Capped to
  // keep peak memory/CPU bounded regardless of installed-connector count — a pod
  // with many connectors won't OOM-kill itself on boot.
  //
  // Note: if a workspace ever declares two connectors that resolve to the same
  ...
  const concurrency = resolveConnectorStartConcurrency();
  ...
  await mapWithConcurrency(flat, concurrency, async ({ wsId, entry }, idx) => {
    ...
    // URL connectors without persisted tokens skip auto-start at boot —
    ...
    // — no surprise OAuth banner on a connector the user added but hasn't
    // authenticated yet. `seedInstance` consults the same token check to
    // set state.
    //
    // A brokered connector's readiness is its provider's answer, not a token file
    ...
    if (
      ...
      log.info(
        `[connectors] Skipping boot start for URL connector "${entry.serverName}" — no tokens yet (state: not_authenticated)`,
      );
      resultEntries[idx] = unstartedUrlConnectorEntry(entry, entry.connector);
      return;
    }
    ...
    try {
      const result = await startConnectorSource(entry.connector, wsRegistry, eventSink, {
        ...
        // never appear for boot-time connectors.
        ...
        ...(opts?.onAuthLost
          ...
          : {}),
        // Re-register inbound host-resources handlers on respawn so connectors
        // installed with host-resources support don't silently lose the
        // capability across platform restarts.
        connectorMcp: opts?.getConnectorMcpDeps?.(entry.wsId),
        ...
        // whether the connector works, and an unregistered source is invisible to
      ...
      // A URL connector that failed to start is still installed, and the failure is
      ...
      // BOTH: the inventory entry survives (see `unstartedUrlConnectorEntry`) so the
      ...
      // `running` — the connector is registered, but its connection is honestly down
      // until a reconnect succeeds.
      //
      // Named/path connectors stay dropped, for two reasons. The decisive one:
      ...
      // connector, which is worse than absence, not better. Second, re-spawning
      // one needs the credential-resolving `startConnectorSource` path that
      // `tryRecoverSource` declines, so the entry would promise a recovery that
      // never comes.
      if ("url" in entry.connector) {
        resultEntries[idx] = unstartedUrlConnectorEntry(entry, entry.connector, msg);
      }
    }
  });

  const finalEntries = resultEntries.filter((e): e is ProcessInventoryEntry => !!e);
  // A failed URL connector survives as an entry (it stays installed), so the
  // started count excludes it explicitly — otherwise this line reads "20/20"
  // while three connectors are dead, and the one boot-time signal that a
  ...
  if (flat.length > 0) {
    ...
    log.info(
      `[workspace-runtime] Started ${finalEntries.length - failed}/${flat.length} connectors in ${elapsedMs}ms (concurrency=${concurrency})${
