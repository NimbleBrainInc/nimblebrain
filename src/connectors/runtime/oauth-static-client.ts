/**
 * Resolve a URL connector's pre-registered OAuth client (Track A) — the one place
 ...
 * Returns `undefined` when the connector has no static client config (the DCR path)
 * — before touching the store, so the overwhelmingly common DCR connector needs no
 ...
 * exactly what the message says. Errors abort the boot of this connector (its
...
export async function resolveStaticOAuthClient(opts: {
  ...
  if (ref.oauthClient.clientSecret) {
    ...
    if (!wrapped) {
      ...
      throw new Error(
        `[connectors] OAuth client_secret not found at credential key "${key}"${forServer} — ` +
