// ---------------------------------------------------------------------------
// URL validation for remote connector connections — SSRF protection.
...
function assertNoEmbeddedCredentials(url: URL): void {
  if (url.username || url.password) {
    throw new Error(
      `Remote connector URL must not contain embedded credentials: ${url.toString()}`,
    );
  }
}
...
function assertNotPrivateAddress(url: URL): void {
  ...
  if (isPrivateHostname(url.hostname) && !isLocalhostHostname(url.hostname)) {
    throw new Error(`Remote connector URL resolves to a private/reserved address: ${url.hostname}`);
  }
}
...
function insecureProtocolMessage(url: URL): string {
  return (
    `Remote connector URL must use HTTPS (got ${url.protocol}//${url.hostname}). ` +
...
function assertAllowedProtocol(
  ...
  throw new Error(`Remote connector URL uses unsupported protocol: ${url.protocol}`);
}

/**
 * Validate a URL for use as a remote connector endpoint.
 *
 * Throws on:
 ...
 */
export function validateConnectorUrl(
  url: URL,
  opts?: { allowInsecure?: boolean; fleetInternal?: boolean },
): void {
