 ...
 * authorization lapse?) over `ConnectorLifecycle` connections — which carry the
...
import type { ConnectionHealthProbe, ConnectionLiveness, ProbeTarget } from "./connection-probe.ts";
import type { ConnectorLifecycleManager } from "./lifecycle.ts";
import type { ConnectorInstance } from "./types.ts";
...
export class ConnectionRevalidator {
  ...
  private async runProbes(
    ...
    await this.forEachBounded(targets, async (t, i) => {
      const probe = this.probes.get(brokeredRef(t.ref)?.provider ?? "");
      if (!probe) {
  ...
  private instanceTargets(inst: ConnectorInstance): ProbeTarget[] {
    const ref = inst.ref;
    const providerId = brokeredRef(ref)?.provider ?? null;
    ...
    if (inst.state === "running") {
      // No per-principal map (shouldn't happen for a running URL connector) —
  ...
  /** Bounded worker pool — inlined to avoid a connectors→runtime import cycle. */
