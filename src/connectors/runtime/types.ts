...
/**
 * One materialized connector-skill overlay bound to a connector. Recorded on
 * the connector's `ConnectorRef.skillsLock` at install so the dedupe path can tell a
 * connector HAS a curated overlay (skip its `skill://<name>/SKILL.md`), and so the binding
...
export type ConnectorRef = {
   ...
   * Set only for connectors whose OAuth provider 302s straight to our
  ...
  /**
   * OAuth scopes the connector requests. Threaded into the provider's
   * `clientMetadata.scope` so the authorize URL carries the right
   * `scope=` param. Surfaces the requested permissions on the review
   * surface (admin reading workspace.json sees what the connector asks
...
/**
 * The brokered coordinates a `ConnectorRef` carries — who brokered this install,
 ...
 * Lives on the URL connector ref. The provider reads `clientId` directly
...
export interface ConnectorInstance {
   ...
   * in-process MCP servers (`defineInProcessApp`), not ConnectorInstance,
  ...
  oauthScope?: "workspace";
  /**
   * Per-principal Connections. Each entry is one (connector, principal)
   * tuple owning an McpSource and its OAuth state machine. For
   * workspace-scoped connectors this map has exactly one entry keyed
  ...
  /**
   * Original `ConnectorRef`, retained on the instance so lifecycle can
...
/** Metadata extracted from a local connector's manifest during startup. */
export interface LocalConnectorMeta {
