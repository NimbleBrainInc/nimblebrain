...
import {
  ...
  type ConnectorMcpDeps,
  composeConnectorMcpContext,
  resolveRefTransport,
...
/** Manifest-derived metadata `seedInstance` accepts for a connector it is seeding,
 *  running or not. */
...
export class ConnectorLifecycleManager {
   ...
   * construction (`setConnectorMcpDepsFactory`). When set, every install
   * path threads the matching deps through `startConnectorSource` so the
   * spawned connector's McpSource registers inbound handlers for
   * `ai.nimblebrain/resources/{read,list}`. Null in test/minimal
   * runtimes that don't wire the host-resources subsystem — connectors
   * spawned in that mode can't call host-resources methods (the
   * handlers are never registered).
   */
  private getConnectorMcpDeps: ((wsId: string) => ConnectorMcpDeps) | null = null;
   ...
   * the connector catalog — the same shape as `setConnectorMcpDepsFactory` above.
   *
   ...
   */
  setConnectorMcpDepsFactory(factory: (wsId: string) => ConnectorMcpDeps): void {
    this.getConnectorMcpDeps = factory;
  }
  ...
  private resolveConnectorMcpDeps(wsId: string): ConnectorMcpDeps | undefined {
    return this.getConnectorMcpDeps?.(wsId);
  }

  // ---- Queries -----------------------------------------------------------

  /** Get a snapshot of all tracked connector instances. */
   ...
   */
  getInstance(serverName: string, wsId: string): ConnectorInstance | undefined {
   ...
   * recorded on the connector's `ConnectorRef.skillsLock` so uninstall can clean up
   ...
   * on uninstall; the `ConnectorRef.skillsLock` itself is dropped with the connector
  ...
  /**
   * Uninstall a connector (PRODUCT_SPEC ss3.4). Every connector is user-removable —
   * there is no `protected` guard.
   *
   * 1. Stop MCP server
   * 2. Remove source from ToolRegistry
   * 3. Emit bundle.uninstalled
  ...
  async uninstall(nameOrPath: string, registry: ToolRegistry, wsId: string): Promise<void> {
    ...
    if (instance) {
      await this.cleanupConnectorCredentials(instance, serverName);
    }
    ...
    // `skillsLock` on the dropped ConnectorRef goes with the config entry above.
   ...
   * `(serverName, wsId)` first; falls back to a `connectorName` match within this
   * workspace. Lookups are always workspace-scoped — uninstalling in one
   * workspace must not affect another workspace's instance of the same connector.
   */
  private resolveUninstallTarget(
    ...
    for (const inst of this.instances.values()) {
      if (inst.wsId === wsId && inst.connectorName === nameOrPath) {
   ...
   */
  private async cleanupConnectorCredentials(
  ...
  /**
   * Tear down a brokered connector's provider-side state on uninstall: the
  ...
  /**
   * Start a stopped connector (re-creates the MCP subprocess).
   * Dead connectors must be explicitly restarted with this method.
   */
  async startConnector(serverName: string, wsId: string, registry: ToolRegistry): Promise<void> {
    const instance = this.instances.get(`${serverName}|${wsId}`);
    if (!instance) {
      throw new Error(`No connector instance found for "${serverName}" in workspace "${wsId}"`);
    }
  ...
  /**
   * Stop a running connector (kills subprocess, keeps source registered).
   */
  async stopConnector(serverName: string, wsId: string, registry: ToolRegistry): Promise<void> {
    const instance = this.instances.get(`${serverName}|${wsId}`);
    if (!instance) {
      throw new Error(`No connector instance found for "${serverName}" in workspace "${wsId}"`);
    }
  ...
  /**
   * Update state on a ConnectorInstance. Public so HealthMonitor can
   * report crashed/recovered/dead transitions.
   */
  transition(instance: ConnectorInstance, newState: ConnectionState): void {
    instance.state = newState;
  }

  /**
   * Record a Connection state transition for a URL connector. Owns:
   *   - Updating the named Connection's state on the ConnectorInstance
   *   - Recomputing `ConnectorInstance.state` via `summarizeConnectionState`
   ...
   * is skipped entirely.
   *
   * Workspace-scoped connectors call with `principalId =
  ...
  /**
   * Initiate (or restart) an OAuth flow for one (connector, principal) tuple.
   *
  ...
  private async startAuthInner(
    ...
    const instance = this.instances.get(`${serverName}|${wsId}`);
    if (!instance) {
      throw new Error(`[lifecycle] connector "${serverName}" not installed in workspace ${wsId}`);
    }
    ...
    const source = new McpSource(
      ...
      this.eventSink,
      composeConnectorMcpContext(this.resolveConnectorMcpDeps(wsId), serverName),
    );
    ...
    // ignoring the `false` is narrower than "the connector is broken": this source
    ...
    // connector was just healed by the concurrent recovery is told to retry, and
  ...
  /**
   * Disconnect one (connector, principal) tuple. Revokes tokens at the AS
  ...
  async disconnect(
    ...
    // A brokered connector's credentials are not among our OAuth records —
  ...
  /**
   * Stop and unwire the McpSource for one (connector, workspace-principal)
   ...
   * `ToolRegistry` type and `startConnectorSource` path; only the owner (identity)
   ...
   * spawn-storm a genuinely broken connector on every tool call; a
   ...
   * No TTL sweep: a key for a since-removed connector lingers until process
   ...
   * by `Runtime.start` after the workspace connector boot loop has constructed
  ...
  /**
   * Register placements from a connector's UI metadata in the PlacementRegistry.
   * Scoped to `wsId` so two workspaces installing the same connector get separate
   ...
   */
  private registerPlacements(serverName: string, ui: ConnectorUiMeta | null, wsId: string): void {
   ...
   * reconstructs the source from the persisted `ConnectorRef` on the
   * `ConnectorInstance` and starts it via `startConnectorSource`.
   *
   ...
   * which already calls `startConnectorSource` internally. The Composio
   ...
   *   - The ConnectorInstance has no URL ref persisted (shouldn't happen
   *     for any path that goes through install)
   *
   * `startConnectorSource` itself can throw on transport / handshake
  ...
  async ensureSourceRegistered(serverName: string, wsId: string, workDir: string): Promise<void> {
    ...
    // terminal to HealthMonitor, leaving the connector worse off than before it was
    ...
    await startConnectorSource(ref, wsRegistry, this.eventSink, {
      ...
      connectorMcp: this.resolveConnectorMcpDeps(wsId),
    });
  }
   ...
   *   3. Start it through the shared `startConnectorSource` path, bound to the
   ...
   * in.
   *
   * `startConnectorSource` can throw on transport / handshake failure; callers
  ...
  async startIdentityAuth(
    ...
    // Reserved-name guard, matching `startConnectorSource` (which the lazy-start
    ...
    try {
      ...
      const source = new McpSource(
        ...
        this.eventSink,
        composeConnectorMcpContext(undefined, serverName),
        // Identity-owned, so it emits its MARKED name. `startConnectorSource` does
   ...
   *   - returns `false` for an unknown instance (connector not installed
   *     here), a non-URL ref (named/stdio re-spawn is the heavier
   *     `startConnectorSource` path — deferred; see the tracked follow-up),
   ...
   *
   * The cooldown is a negative cache: a genuinely-broken connector (bad
  ...
  async tryRecoverSource(serverName: string, wsId: string, workDir: string): Promise<boolean> {
    ...
    // on this path. No instance → the connector isn't installed in this
    ...
    if (last !== undefined && now - last < ConnectorLifecycleManager.RECOVERY_COOLDOWN_MS) {
      return false;
    }
  ...
  /**
   * Seed instances from the initial connector startup, and from the install path.
   *
   * A seeded connector is not necessarily a running one: a URL connector that was
  ...
  async seedInstance(
    ...
    wsId: string,
    /** Boot-start failure message for an installed-but-not-running connector.
    ...
    this.instances.set(
      `${serverName}|${wsId}`,
      buildSeededInstance(serverName, connectorName, ref, manifestMeta, wsId),
    );
    await this.seedUrlConnectionState(serverName, wsId, ref, startError);
  }

  /**
   * Derive and record the boot-time Connection state for a seeded URL connector.
...
/**
 * Build the `ConnectorInstance` `seedInstance` records.
 *
