 ...
 * [`ServerDetail`](../../src/connectors/catalog/schemas/server.schema.json), and any
...
import type { HostManifestMeta } from "../runtime/types.ts";
...
/** Stdio transport (no command/args at the wire — those live on the connector). */
...
export interface NimbleBrainConnectorMeta {
   ...
   * Operator-authored; the install path copies it verbatim into the ConnectorRef
  ...
  /** Optional OAuth scopes the connector requests. */
