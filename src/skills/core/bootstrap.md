...
- **Instructions** — Persist standing guidance with `instructions__write_instructions`. It applies to every conversation in this workspace. **This is how you remember something.** Reach for it when the user says "remember this", "save this", "from now on", or hands you conventions to keep. Confirm with the user before writing, and say where it landed. For guidance that should reach every workspace in the organization, author an org-tier skill instead. Search query: `"instructions"`
