#!/usr/bin/env bun
/**
 * Lint: a platform app's UI must use @nimblebrain/synapse for postMessage envelopes.
 *
 * Platform app UIs (`src/platform/<name>/ui/src/`) talk to the host iframe
 * bridge via the SDK. The bridge validates every inbound envelope against
 ...
 *   - A `// lint-ok:platform-app-transport` marker on the line immediately
 ...
 *     ext-apps spec.
 *
 * Scope: only `src/platform/* /ui/**` is scanned. The rest of an app —
 * its source and its tool handlers — doesn't postMessage to anyone.
 */
...
const APPS_ROOT = join(ROOT, "src/platform");
const ALLOW_MARKER = "lint-ok:platform-app-transport";
...
async function main(): Promise<void> {
  ...
  for await (const rel of glob.scan({ cwd: APPS_ROOT })) {
    const abs = join(APPS_ROOT, rel);
  ...
  if (violations.length > 0) {
    console.error(`✗ Found ${violations.length} raw postMessage call(s) in platform app UIs:\n`);
    for (const v of violations) {
      console.error(`  ${v.file}:${v.line}:${v.column}`);
      console.error(`    ${v.snippet}\n`);
    }
    console.error(
      "A platform app UI must construct postMessage envelopes via @nimblebrain/synapse.",
    );
  ...
  console.log(`✓ No raw postMessage in ${scanned} platform app UI files`);
}
