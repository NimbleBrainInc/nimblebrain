...
describe("SseEventManager — identity-scoped clients", () => {
  ...
  test("a broadcast given a wsId reaches members of that workspace only", async () => {
    // `broadcast`'s third argument is the membership gate, independent of the
  ...
  test("a broadcast with NO wsId reaches everyone", async () => {
    ...
    expect(alice.events).toEqual(["config.changed"]);
    expect(bob.events).toEqual(["config.changed"]);
  });
