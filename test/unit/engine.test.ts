...
/** Build a ToolResult with _taskResult metadata (mirrors tasks.test.ts helper). */
function makeTaskToolResult(opts?: { pollInterval?: number }): ToolResult {
  return {
    content: textContent("Task created"),
    isError: false,
    _taskResult: {
      task: {
        taskId: "timeout-task-001",
        status: "working",
        ttl: null,
        createdAt: "2025-01-01T00:00:00Z",
        lastUpdatedAt: "2025-01-01T00:00:00Z",
        pollInterval: opts?.pollInterval ?? 10,
      },
    },
  };
}

/** A TaskClientPort whose getTask() never resolves (simulates a hung API). */
function makeHangingTaskClient(): TaskClientPort {
  return {
...
/** A TaskClientPort that resolves immediately with a completed task. */
function makeFastTaskClient(): TaskClientPort {
  return {
...
describe("AgentEngine task polling timeout", () => {
  it("returns error result when task poll exceeds taskTimeoutMs", async () => {
    let callCount = 0;
    const model = createMockModel(() => {
      callCount++;
      if (callCount === 1) {
      ...
      return {
        content: [{ type: "text", text: "Handled timeout" }],
        inputTokens: 10,
        outputTokens: 5,
      };
    });
    ...
    const tools = {
      ...
      handler: (): ToolResult => makeTaskToolResult({ pollInterval: 5 }),
    };
    ...
    const result = await engine.run(
      {
        ...defaultConfig,
        taskTimeoutMs: 50,
        taskClientResolver: () => makeHangingTaskClient(),
      },
      "",
      [{ role: "user", content: [{ type: "text", text: "Go" }] }],
      tools.schemas,
    ...
    expect(result.toolCalls).toHaveLength(1);
    expect(result.toolCalls[0]!.ok).toBe(false);
    expect(result.toolCalls[0]!.output).toContain("timed out");
    expect(result.toolCalls[0]!.output).toContain("50ms");
  });

  it("engine resolves (does not hang) after task timeout", async () => {
    let callCount = 0;
    const model = createMockModel(() => {
      callCount++;
      if (callCount === 1) {
        return {
          content: [
            { type: "tool-call", toolCallId: "c1", toolName: "test__hung", input: JSON.stringify({}) },
          ],
          inputTokens: 10,
          outputTokens: 5,
        };
    ...
    const tools = {
      schemas: [{ name: "test__hung", description: "Hangs", inputSchema: {} }],
      handler: (): ToolResult => makeTaskToolResult({ pollInterval: 5 }),
    };
    ...
    // Engine must complete — not hang
    expect(result.stopReason).toBe("complete");
    // Should have finished within a generous bound (timeout + LLM overhead)
    expect(elapsed).toBeLessThan(500);
  });

  it("does not timeout when task completes within budget", async () => {
    let callCount = 0;
    const model = createMockModel(() => {
      callCount++;
      if (callCount === 1) {
        return {
          content: [
            { type: "tool-call", toolCallId: "c1", toolName: "test__fast", input: JSON.stringify({}) },
          ],
          inputTokens: 10,
          outputTokens: 5,
        };
    ...
    const tools = {
      schemas: [{ name: "test__fast", description: "Fast task", inputSchema: {} }],
      handler: (): ToolResult => makeTaskToolResult({ pollInterval: 10 }),
    };
    ...
    const result = await engine.run(
      {
        ...defaultConfig,
        taskTimeoutMs: 500,
        taskClientResolver: () => makeFastTaskClient(),
      },
      "",
      [{ role: "user", content: [{ type: "text", text: "Go" }] }],
      tools.schemas,
    ...
    expect(result.toolCalls).toHaveLength(1);
    expect(result.toolCalls[0]!.ok).toBe(true);
    expect(result.toolCalls[0]!.output).toBe("Fast result");
  });

  it("llm.done event includes content blocks from the model response", async () => {
    const expectedContent = [{ type: "text" as const, text: "Hello from LLM" }];
    const model = createMockModel(() => ({
    ...
    expect(llmDone!.data["content"]).toEqual(expectedContent);
  });
});
...
describe("malformed tool call input", () => {
  ...
  it("no taskClient falls through with placeholder result", async () => {
    let callCount = 0;
    const model = createMockModel(() => {
      callCount++;
      if (callCount === 1) {
    ...
    const tools = {
      schemas: [{ name: "test__task", description: "Task tool", inputSchema: {} }],
      handler: (): ToolResult => makeTaskToolResult(),
    };
    ...
    // No taskClientResolver configured — should fall through with placeholder
    const result = await engine.run(
      { ...defaultConfig },
      "",
      [{ role: "user", content: [{ type: "text", text: "Go" }] }],
