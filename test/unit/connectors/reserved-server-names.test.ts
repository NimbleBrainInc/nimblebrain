 ...
 *
 * `validateServerName` is NOT install-only. `src/connectors/runtime/startup.ts` calls it on
 * every URL / named / local connector start (three sites), so it re-validates
