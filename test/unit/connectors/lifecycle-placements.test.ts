...
/**
 * Placements ride on the connector's `ConnectorRef.ui`, copied there at install
...
function refWithUi(placements?: PlacementDeclaration[]): ConnectorRef {
	return {
		url: URL,
		serverName: SERVER,
...
async function seedAndNotify(lifecycle: ConnectorLifecycleManager, ref: ConnectorRef): Promise<void> {
	await lifecycle.seedInstance(SERVER, URL, ref, undefined, WS);
	lifecycle.notifyInstalled(SERVER, WS);
}

describe("ConnectorLifecycleManager — placement registration on install", () => {
