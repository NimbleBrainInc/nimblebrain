...
/**
 * A remote connector carrying its own credential (`provider` auth) — the fleet
 ...
 * otherwise.
 */
function fleetRef(): ConnectorRef {
  return {
...
function connectionOf(lifecycle: ConnectorLifecycleManager) {
  return lifecycle.getInstance(SERVER, WS)?.connections?.get("_workspace");
}

describe("boot-start failure — seeding an installed-but-not-running URL connector", () => {
  ...
  test("seedInstance_withoutStartError_stillRecordsRunning", async () => {
    // The static-auth connector auto-connects, so a clean boot must still land on
