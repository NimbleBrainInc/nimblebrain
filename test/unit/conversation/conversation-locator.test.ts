...
test("workspace-scoped list returns only the focused workspace; all-workspaces returns every workspace", async () => {
  ...
  const workspaceScoped = await loc.list({ kind: "workspace", workspaceId: "ws_helix" }, {}, access);
  expect(workspaceScoped.conversations.map((c) => c.id)).toEqual([helix]);

  const allWorkspaces = await loc.list({ kind: "all-workspaces" }, {}, access);
...
test("the access gate hides another owner's conversation in the same workspace", async () => {
  ...
  const loc = locator();
  const aliceList = await loc.list({ kind: "workspace", workspaceId: "ws_helix" }, {}, { userId: "usr_alice" });
...
test("invalidate + JIT rescan picks up a newly written conversation (no fs.watch)", async () => {
  ...
  // Cold read populates.
  expect((await loc.list({ kind: "all-workspaces" }, {}, { userId: "usr_alice" })).totalCount).toBe(1);
  ...
  loc.invalidate();
  expect((await loc.list({ kind: "all-workspaces" }, {}, { userId: "usr_alice" })).totalCount).toBe(2);
});
...
test("an ownerless file is excluded from list() but still resolves by path", async () => {
  ...
  expect((await loc.list({ kind: "all-workspaces" })).totalCount).toBe(0);
