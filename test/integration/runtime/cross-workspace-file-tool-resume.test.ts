 ...
 * workspace-owned store from `RequestContext.focusedWorkspaceId`, which `chat()` sets
 ...
 * in this path (e.g. `focusedWorkspaceId` reverting to `request.workspaceId ??
 * sessionWsId`, or the identity-tool-router dropping `focusedWorkspaceId` from its
...
describe("cross-workspace resume scopes the file TOOL to the conversation's workspace (not the request)", () => {
  it("files__list on an UNFOCUSED resume reads workspace A's partition and finds the attachment", async () => {
    ...
    //    RequestContext.focusedWorkspaceId.
