...
- Long-running MCP tasks no longer surface as `MCP error -32603: Task <id> failed` when the server-side task was misclassified after producing a real result. The engine now refetches `tasks/result` whenever the SDK reports the generic `Task <id> failed` shape (the upstream stream discards the payload on status=failed). Bundle-specific error messages pass through unchanged. Defense-in-depth for the synapse-research production failure where research runs completed cleanly to the entity store but came back to the agent as a useless error string.
- `automations__run` no longer fails with `MCP error -32001: Request timed out` on multi-minute automations. `handleRun` now caps its synchronous wait at 30 s (well below the SDK's 60 s request timeout) and returns a `{ status: "dispatched", automationId, message }` envelope when the run outlasts the window. The scheduler keeps tracking the run; callers poll `automations__runs` for the final record. Quick automations still return their full run record synchronously.
- Synthesized automation failure records carry an honest elapsed window. `Scheduler.dispatchRun`'s catch path was using `new Date()` for both `startedAt` and `completedAt`, making a 5-minute hang and a 100-millisecond setup crash render identically. `startedAt` is now captured before awaiting the executor; operators can tell the failure modes apart from the run record alone.
