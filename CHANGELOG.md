...
- Task-augmented MCP tools no longer surface as `MCP error -32603: Task <id> failed` when the upstream stream's status=failed branch discarded a usable `tasks/result` payload — the engine now refetches the result and recovers the real content ([#245](https://github.com/NimbleBrainInc/nimblebrain/pull/245)).
