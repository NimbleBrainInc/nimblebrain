...
- Default thinking config now works on Claude Opus 4.7. Anthropic rejects `thinking.type=enabled` for that model with a specific error pointing at `output_config.effort`; the engine now translates the platform's enabled+budget into `thinking.type=adaptive` plus a top-level `effort` tier mapped from the budget (≤4K→low, ≤16K→medium, ≤32K→high, else max). Adaptive-only model gating lives in `supportsEnabledThinking()` so future Anthropic models that share the constraint are a one-line addition.
